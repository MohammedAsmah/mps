<?php
	...
	$action="recherche";$client=$_GET["client"];
...
<? 	
	...
	$sql .= "FROM porte_feuilles where client='$client' and m_cheque<>0 and remise=0 and chq_f=1 and date_cheque>'$dd' GROUP BY numero_cheque;";
	$users11 = db_query($database_name, $sql);
	
?>


<span style="font-size:24px"><?php echo "Porte Feuilles Cheques : ".$client; ?></span>
...
while($users_1 = fetch_array($users11)) { 
			...
			$v_banque=$users_1["v_banque"];$numero_cheque=$users_1["numero_cheque"];$total_cheque=$users_1["total_cheque"];$date_cheque=$users_1["date_cheque"];
