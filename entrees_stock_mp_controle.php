<?php
	...
	if($_REQUEST["action_"] == "insert_new_user") {	
		...
		if($_REQUEST["depot_a"] != 0) {	
				$sql  = "INSERT INTO entrees_stock_mp_controle ( produit, date,frs,reception,commande,livraison,depot_a,type,depot_b ) VALUES ( ";
		...
		if($_REQUEST["depot_a9"] != 0) {	
			...
			$sql = "UPDATE entrees_stock_mp_controle SET ";
			...
			$sql = "DELETE FROM entrees_stock_mp_controle WHERE id = " . $_REQUEST["user_id"] . ";";
			db_query($database_name, $sql);
			break;
	...
	?>
	<form id="form" name="form" method="post" action="entrees_stock_mp_controle.php">
	...
	$sql  = "SELECT * ";$type="reception";$date=dateFrToUs($_POST['date']);
	$sql .= "FROM entrees_stock_mp_controle where date='$date' and type='$type' ORDER BY date;";
	$users = db_query($database_name, $sql);}
	else
	{	$sql  = "SELECT * ";$type="reception";$date=dateFrToUs($_POST['date']);
	$sql .= "FROM entrees_stock_mp_controle where type='$type' and date='$date' ORDER BY date;";
	$users = db_query($database_name, $sql);}
...
<script type="text/javascript"><!--
	function EditUser(user_id) { document.location = "entree_stock_mp_controle.php?user_id=" + user_id; }
--></script>
...
<span style="font-size:24px"><?php echo " RECEPTION "; ?></span>
...
<td style="text-align:left"><?php echo number_format($users_["depot_a"],2,',',' '); ?></td>
...
<table><tr><td>
<? if ($date<>""){echo "<a href=\"entree_stock_mp_controle.php?date=$date&user_id=0\">Ajout Reception</a></td>";}?></tr><tr>
