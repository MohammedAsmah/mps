<?php
	...
	if(isset($_REQUEST["action_"]) ) { 
		...
		if($produit <> "") {
				...
				$sql  = "INSERT INTO entrees_stock ( produit, date,date_ins,depot_b,type,depot_a ) VALUES ( ";
				$sql .= "'" . $produit . "', ";
				$sql .= "'" . $date . "', ";
				$sql .= "'" . $date_ins . "', ";
				$sql .= "'" . $depot_a . "', ";
...
--></script>
	...
	<form id="form_user" name="form_user" method="post" action="entrees_stock_barcode_jp_g.php">
	
	<div>
	<h1 style="color:blue;"><?php echo $date_ins." ---->PRODUCTION vers JAOUDA"; ?></h1>
...
<script>
  $('#barecode').keyup(function(){
	...
	$sql .= "FROM entrees_stock where type='$type' and date='$dj' group by produit ORDER BY date_ins;";
...
<? $user_id=$users_["id"];$date1=dateUsToFr($users_["date"]);$produit=$users_["produit"];
$sql  = "SELECT * ";
	$sql .= "FROM produits where produit='$produit' GROUP BY id ORDER BY id;";
	$usersss = db_query($database_name, $sql);
    $usersss_ = fetch_array($usersss);$photo=$usersss_["image"];
?>
<td style="text-align:left"><?php echo $date1; ?></td>
<td style="text-align:left"><?php echo $users_["produit"]; ?></td>
<td><div class="icon" >
<img src="<?php echo $photo ?>" height="150px" width="150px">
</div></td>
