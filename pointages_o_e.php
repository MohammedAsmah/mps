<?php
	...
	{
?>
	<form id="form" name="form" method="post" action="pointages_o_e.php">
	...
	$sql  = "SELECT id,name,date,sum(heures) as t_heures,sum(lun+mar+mer+jeu+ven+sam) as normales ,sum(lun_s+mar_s+mer_s+jeu_s+ven_s+sam_s) as sup, sum(dim) as t_dim,sum(dim_s) as t_dim_s ";
...
<td><?php $ss=$users_["sup"]+$users_["t_dim_s"]+$users_["t_dim"];$s1=$users_["dim_s"];$s2=$users_["t_dim_s"];
$retenue=0;$no=$users_["normales"];if ($users_["normales"]<44){$diff=44-$users_["normales"];
if ($diff<=$ss){$supp=$ss-$diff;$retenue=$diff;}else{$retenue=$ss;$supp=0;}}echo $no+$retenue; ?></td>
<td><?php echo $supp; ?></td>
<?php } ?>
