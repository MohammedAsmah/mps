<?php
	...
	?>
	<form id="form" name="form" method="post" action="evaluations_client_non_sortie.php">
	...
	{$date=dateFrToUs($_POST['date']);$date_f=dateFrToUs($_POST['date']);$date1=dateFrToUs($_POST['date1']);
		...
		$sql  = "SELECT * ";
		$sql .= "FROM commandes where id_registre=0 and (date_e between '$date' and '$date1') ORDER BY date_e;";
		$users = db_query($database_name, $sql);
		}
		
		else
		{$sql  = "SELECT * ";
		$sql .= "FROM commandes where id_registre=0 and date_e='$date' ORDER BY date_e;";
		$users = db_query($database_name, $sql);}
?>
...
<span style="font-size:24px"><?php echo "liste Evaluations non sorties du $du1 au $au1"; ?></span>
<tr>
</tr>
