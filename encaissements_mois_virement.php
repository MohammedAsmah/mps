<?php
	...
	<form id="form" name="form" method="post" action="encaissements_mois_virement.php">
...
<? 	 if(isset($_REQUEST["action"]))
	{ $date=dateFrToUs($_POST['date1']);$total=0;$date2=dateFrToUs($_POST['date2']);$total=0;
	...
	$sql .= "FROM porte_feuilles where facture_n<>0 and m_virement<>0 and id_registre_regl<>0 and date_enc between '$date' and '$date2' group by id ORDER BY date_echeance;";
	$users11 = db_query($database_name, $sql);
	
?>


<span style="font-size:24px"><?php echo "Etat des Virements reçus du : ".$_POST['date1']." Au ".$_POST['date2']; ?></span>
...
while($users_1 = fetch_array($users11)) { $date_enc=$users_1["date_enc"];$vendeur=$users_1["vendeur"];$impaye=$users_1["impaye"];
			$ref=$users_1["numero_virement"];$facture_n=$users_1["facture_n"];$date_remise=$users_1["date_remise"];
			...
			$total_cheque=$users_1["total_cheque"];$total_espece=$users_1["total_espece"];$total_virement=$users_1["total_virement"];
	...
	$users111 = db_query($database_name, $sql);$user_ = fetch_array($users111);
		$inputation=$user_["inputation"];$patente=$user_["patente"];
			...
			<td align="right"><?php $total_c=$total_c+$total_virement;$tchq=number_format($total_virement,2,',',' ');
