<?php
	...
	// recherche contrat			
		$code_produit="";
			...
			if(isset($_REQUEST["valide"])) { $valide = 1; 
			if ($valide==1){
...
//////////////////////////////blocage evaluation
	...
	while($users1_ = fetch_array($users1)) { $id=$users1_["id"];
		...
		$sql  = "SELECT * ";
		$sql .= "FROM fiches_techniques WHERE accessoire<>'produit' and id_produit = '$id_produit' ;";
...
//promotion
	...
	$produit=$users11_["produit"]; $quantite=$users11_["total_quantite"];$condit=$users11_["condit"];
				...
				$sql  = "INSERT INTO bon_de_sortie_magasin ( id_registre, date,produit, condit,depot_b ) VALUES ( ";
				$sql .= "'" . $id_registre . "', ";
				$sql .= "'" . $date_v . "', ";
				$sql .= "'" . $produit . "', ";
				$sql .= "'" . $condit . "', ";
	...
	<form id="form" name="form" method="post" action="registres_vendeurs_s.php">
...
while($users_1 = fetch_array($users11)) { $id_r=$users_1["id"];$date3=$users_1["date"];$vendeur=$users_1["vendeur"];
			$statut=$users_1["statut"];$observation=$users_1["observation"];$valide_b=$users_1["valide"];$valide_c=$users_1["valide_c"];
			$service=$users_1["service"];$code=$users_1["code_produit"];$lp=$users_1["id"]+100000;$bon=$users_1["statut"];$ttg=0;?><tr>
			<? 
			if ($valide_c==0){
			if ($valide_b==1){?>
			...
			<td><?php echo $bon; ?></td>
				<td><?php $vendeur=$users_1["vendeur"];echo $users_1["vendeur"]; ?></td>
					<? 
	echo "<td><a href=\"bon_de_sortie55_g.php?observation=$observation&montant=$ttg&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">".$bon."</a></td>"; 
...
<? if ($login=="admin"){
echo "<table<td><a href=\"bon_de_sortie_journee.php?date=$date\">RECAP</a></td></table>";
}
?>
