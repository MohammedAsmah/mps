...
$pdf->addText(470, 695, 14, 'Observation');
	...
	$sql1  = "SELECT produit,condit,sum (quantite) As qte,sum (quantite*condit) As qte_t ";
	$sql1 .= "FROM  bon_de_sortie where commande='$id_registre' GROUP by produit;";
	$users1 = db_query($database_name, $sql1);$non_favoris=0;
	while($users1_ = fetch_array($users1)) { 
		$qte=$users1_["qte"];$produit=$users1_["produit"];$condit=$users1_["condit"];
