<?php
	...
	while($temp_ = fetch_array($temp)) {
		if($vendeur == $temp_["client"]) { $selected = " selected"; } else { $selected = ""; }
	...
	?>
	<form id="form" name="form" method="post" action="evaluations_client_par_vendeur.php">
...
<link rel="stylesheet" type="text/css" href="styles.css">
	...
	<? {$date=dateFrToUs($_POST['date']);$date_f=dateFrToUs($_POST['date']);$date1=dateFrToUs($_POST['date1']);
		...
		$sql  = "SELECT * ";
		$sql .= "FROM commandes where date_e between '$date' and '$date1' ORDER BY date_e;";
		$users = db_query($database_name, $sql);
		
?>


<span style="font-size:24px"><?php echo "liste Evaluations Clients du : $du1 au $au1"; ?></span>
<tr>
</tr>
