...
set_time_limit(0);
	...
	if(isset($_REQUEST["action_"])) { 
		if($_REQUEST["action_"] != "delete_user") {
			...
			$date_v = dateFrToUs($_REQUEST["date"]);$date_fr = $_REQUEST["date"];
	...
	// recherche contrat			
		$code_produit="";
				...
				$type_service="SEJOURS ET CIRCUITS";$bl_out_d=1;
				$sql  = "INSERT INTO registre_vendeurs (date,datefr,service,destination,vendeur,date_open,user_open,observation,livraison,mois,annee,bon_sortie,bl_out,statut)
				 VALUES ('$date_v','$date_fr','$service','$service','$vendeur','$date_open','$user_open','$observation','$livraison','$mois','$annee','$dir','$bl_out_d','$statut')";
			...
			$sql .= "destination = '" . $_REQUEST["service"] . "', ";
			$sql .= "bl_out = '" . $bl_out . "', ";
			$sql .= "bl_in = '" . $bl_in . "', ";
	...
	<form id="form" name="form" method="post" action="registres_vendeurs.php">
...
<? 	if(isset($_REQUEST["action"]))
	...
	$sql  = "SELECT * ";$d1="2021-11-01";$d2="2021-10-31";
...
<tr>
	...
	<th><?php echo "Eval Vendeur1";?></th>
	<th><?php echo "Eval Vendeur2";?></th>
	<th><?php echo "Montant";?></th>
	<th><?php echo "B.S Gardien";?></th>
	<th><?php echo "B.F Transport";?></th>
	<th><?php echo "B.S Magasin";?></th>
	<th colspan="5"><?php echo "Eval.Clients";?></th>
	
	
</tr>

<?php $compteur1=0;$total_g=0;
while($users_1 = fetch_array($users11)) { $id_r=$users_1["id"];$date3=$users_1["date"];$vendeur=$users_1["vendeur"];$date_fr=dateUsToFr($users_["date_e"]);
			...
			$datefr=dateUsToFr($users_1["date"]);
			/*$sql = "UPDATE registre_vendeurs SET ";
			
			$sql .= "datefr = '" . $datefr . "', ";
			$sql .= "destination = '" . $service . "' ";
			$sql .= "WHERE id = " . $id_r . ";";
			db_query($database_name, $sql);*/
			
			$sql  = "SELECT id_registre,sum(net) as total_net ";
	$sql .= "FROM commandes where id_registre=$id_r group by id_registre ORDER BY date_e;";
	$users = db_query($database_name, $sql);
	$users_ = fetch_array($users);
	$total_g=0;$ttg=0;
	$ttg=$users_["total_net"];
	...
	/*while($users_ = fetch_array($users)) { 
		...
		$id=$users_["id"];
			...
			if ($valide==1){?><td bgcolor="#33FFCC">
						<? echo "<a href=\"registre_vendeur.php?user_id=$user_id&montant=$ttg&frais=$frais&fv=$frais_v\">".$dt."</a>"; ?></td>
	...
	<? 
			...
			<td><?php $impression=$hh."-->".$mm;if ($imprimer)
			{ echo "<a href=\"evaluation_pre5.php?impression=$impression&observation=$observation&montant=$ttg&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">".$vendeur."</a>"; 
			
			$imp=" Entete"; $entete="<a href=\"evaluation_pre_imp.php?observation=$observation&montant=$ttg&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">".$imp."</a>"; 
			print("<font size=\"1\" face=\"Comic sans MS\" color=\"#FF0000\">$entete </font>");}
			?>
			</td>
			 <td align="right"><? echo number_format($ttg,2,',',' ')."</td>";?>
 			
			<td><?php if ($destination<>""){if ($id_r==6162){$ttg="57700.00";}
			...
			<?php if ($user_login=="admin"){
			echo "<td><a href=\"bon_de_sortie_couleurs.php?observation=$observation&montant=$ttg&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">Bon de sortie couleurs</a></td>";} ?>
			...
			echo "<td><a href=\"evaluations.php?id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon\">"."Ajout"."</a></td>";
			echo "<td><a href=\"evaluations1.php?valide=$valide&bon_sortie=$bon&destination=$destination&id_registre=$id_r&date=$date3&vendeur=$vendeur\">"."Modif"."</a></td>";
			}else
			{				
			echo "<td>Validee</td>";
			echo "<td></td>";
			}
			//if ($user_login=="admin"){echo "<td><a href=\"editer_evaluation_vendeur_ent_g.php?numero=$id_r\">"."Imprimer Ev."."</a></td></tr>";	}
			echo "<td><a href=\"editer_evaluation_vendeur_ent_g.php?numero=$id_r\">"."Imprimer Ev."."</a></td></tr>";
 
 //mise ajour net registre vendeurs json_decode
...
<? echo "<td><a href=\"registre_vendeur.php?date=$date&user_id=0\">"."Ajout Evaluation Vendeur"."</a></td>";?>
