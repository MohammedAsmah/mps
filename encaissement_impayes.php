<?php
	...
	$id_secours=$_GET['id_secours'];$ref_impaye=$_GET['ref_impaye'];$date_f=$_GET['date_f'];$client=$_GET['client'];
	$du1=$_GET['du'];$au1=$_GET['au'];
	if ($idp==0){
	$sql  = "SELECT * ";
	$sql .= "FROM porte_feuilles_factures where id='$id_secours' ORDER BY id;";
	}else
	{	
	$sql  = "SELECT * ";
	$sql .= "FROM porte_feuilles_factures where id_porte_feuille='$idp' ORDER BY id;";
	}
	...
	$users11 = db_query($database_name, $sql);$row = fetch_array($users11);
			...
			$numero_cheque_imp=$row["numero_cheque"];$v_banque_imp=$row["v_banque"];
			$m_cheque=$row["m_cheque"];
			$date_impaye=$row["date_impaye"];//$facture_n=$row["facture_n"];
	if ($action_s=="sup")
...
<table class="table2"><tr><td style="text-align:center">
	...
	<table class="table3">
		...
		<tr>
		<td><?php echo "Numero cheque "; ?></td><td><?php echo $numero_cheque_imp." ".$v_banque_imp; ?></td>
		</tr>
		<tr>
		<td><?php echo "Montant cheque "; ?></td><td><?php echo $m_cheque; ?></td>
		</tr>
		<tr>
...
while($users_1 = fetch_array($users11)) { 
	...
	$m_avoir1=$users_1["m_avoir"];$numero_avoir1=$users_1["numero_avoir"];$numero_chq=$users_1["numero_cheque"];
	...
	$row1 = db_query($database_name, $sql);$row = fetch_array($row1);
			...
			?><tr>
						<td><?php $sup="sup";$n_v1="<a href=\"encaissement_impayes.php?facture_n=$facture_n&date_f=$date_f&numero=$numero_chq&action_s=$sup&du=$du1&au=$au1&idp=$idp&id_s=$id_s\">supprimer</a>";
...
<tr><td><td><td><?php print("<font size=\"2\" face=\"Comic sans MS\" color=\"#000033\">Solde </font>"); ?></td>
    ...
    <td><input name="espece" type="text" class="Style1" id="espece" style="width:140px" value="<?php echo $espece; ?>"></td></tr>
    ...
    <td><input name="m_virement" type="text" class="Style1" id="m_virement" style="width:140px" value="<?php echo $m_virement; ?>"></td></tr>
    ...
    <td><input name="numero_effet" type="text" class="Style1" id="numero_effet" style="width:140px" value="<?php echo $numero_effet; ?>"></td></tr>
	<tr><td bgcolor="#3399FF" width="150">Montant Effet : </td>
    <td><input name="m_effet" type="text" class="Style1" id="m_effet" style="width:140px" value="<?php echo $m_effet; ?>"></td></tr>
	...
	<td><input name="v_banque1" type="text" class="Style1" id="v_banque1" style="width:140px" value="<?php echo $v_banque1; ?>"></td>
