...
<? 	
	...
	$date=dateFrToUs(date("d/m/Y"));$total=0;$vide="0000-00-00";$dd="2024-03-30";
	
	$sql  = "SELECT id,date_enc,date_cheque,client,client_tire,v_banque,numero_cheque,sum(m_cheque) As total_cheque ";
	$sql .= "FROM porte_feuilles where m_cheque<>0 and remise=0 and chq_f=1 GROUP BY id order by date_cheque;";
...
while($users_1 = fetch_array($users11)) { 
			...
			<? echo "<tr><td>$date_cheque</td>";
			echo "<td>$numero_cheque</td>";
			echo "<td>$client</td>";
			echo "<td>$client_tire</td>";
...
?>
<? } ?>
<tr><td></td><td></td><td></td><td></td><td align="right"><? $m=number_format($total_g,2,',',' ');echo "<a href=\"cheques_non_remis.php\">$m</a></td>";?>
<tr><? 
