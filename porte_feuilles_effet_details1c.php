<?php
	...
	$action="recherche";$client=$_GET["client"];$date=$_GET["date"];
...
<? 	
	...
	$sql  = "SELECT id,date_enc,date_echeance,client,client_tire,client_tire_e,v_banque,numero_effet,sum(m_effet) As total_cheque ";
	$sql .= "FROM porte_feuilles where date_echeance<='$date' and client='$client' and m_effet<>0 and remise_e=0 and eff_f=1 GROUP BY numero_effet;";
	$users11 = db_query($database_name, $sql);
	
?>


<span style="font-size:24px"><?php echo "Porte Feuilles Facturé  : ".$client." au ".dateUsToFr($date); ?></span>
...
while($users_1 = fetch_array($users11)) { 
			...
			<? echo "<td><a href=\"maj_date_effet.php?client=$client&ref=$ref&id=$id&date=$date\">$ref1</a></td>";?>
