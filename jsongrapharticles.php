...
<?php
	...
	$mois = $date->format('m');
	$annee = $date->format('y');
	echo $mois;echo $annee;
	$produit=$_GET['article'];$mois = $date->format('m');$annee = $date->format('Y');
    $sql  = "SELECT produit,sum(quantite) As quantite ";
	$sql .= "FROM detail_commandes where MONTH(date) ='$mois' and YEAR(date) ='$annee' and escompte_exercice=0 GROUP BY produit order by id;";
