...
//gets the login
	...
	if(isset($_REQUEST["action_"]) && $profile_id == 1) { 
		...
		switch($_REQUEST["action_"]) {
			...
			case "insert_new_user":
				...
				$result = mysql_query("SELECT commande FROM commandes_test ORDER BY commande DESC LIMIT 0,1"); 
				...
				$result = mysql_query("SELECT eval FROM commandes_test where (mois='$mois1' and annee='$annee') ORDER BY eval DESC LIMIT 0,1"); 
				$row = mysql_fetch_array($result); 
				$dir_eval = $row["eval"]+1;
				...
				$sql  = "INSERT INTO commandes_test ( commande,date_e,client,secteur,company, type_remise,escompte,escompte2,plafond,vendeur, remise_10,remise_2,remise_3,evaluation,destination,sans_remise ) VALUES ( ";
				$sql .= "'" . $cde . "', ";
				$sql .= "'" . $date . "', ";
				...
				if ($client1<>""){
						...
						$result = mysql_query("SELECT commande FROM commandes_test ORDER BY commande DESC LIMIT 0,1"); $escompte1_2 = $user_["escompte2"];$plafond1 = $user_["plafond"];
						$row = mysql_fetch_array($result); 
						$dir = $row["commande"];
				...
				if ($dir_eval>=100){$dev=$dir_eval;}
				$encours=$mois1." ".$dev;$cde=$dir+1;$encours="encours";
				$sql  = "INSERT INTO commandes_test ( commande,date_e,client,secteur, company,vendeur, remise_10,remise_2,remise_3,escompte,escompte2,plafond,evaluation,destination,sans_remise ) VALUES ( ";
						$sql .= "'" . $cde . "', ";
						$sql .= "'" . $date1 . "', ";
		...
		$date=$_POST['date'];$vendeur=$_POST['vendeur'];$action="recherche";$destination=$_POST['destination'];
			...
			$net = $_REQUEST["net"];if(isset($_REQUEST["active"])) {$active = $_REQUEST["active"];}
				...
				if ($mois=="Mar"){$mois1="MARS";}
				if ($mois=="Apr"){$mois1="AVRIL";}
				$result = mysql_query("SELECT eval FROM commandes_test where (mois='$mois1' and annee='$annee') and active=1 ORDER BY eval DESC LIMIT 0,1"); 
				$row = mysql_fetch_array($result); 
				$dir_eval = $row["eval"]+1;
				if ($dir_eval<10){$dev="00".$dir_eval;}
				if ($dir_eval>9 and $dir_eval<100) {$dev="0".$dir_eval;}
			...
			$sql  = "SELECT * ";
			$sql .= "FROM commandes_test WHERE id = " . $_REQUEST["user_id"] . ";";
			...
			if ($encours<>"" and $encours<>"encours")
				{
					$sql = "UPDATE detail_commandes_test SET ";
					$sql .= "date = '" . $date . "', ";
					$sql .= "evaluation = '" . $encours . "' ";
			...
			$sql = "DELETE FROM commandes_test WHERE id = " . $_REQUEST["user_id"] . ";";
			db_query($database_name, $sql);
			
			$sql = "DELETE FROM detail_commandes_test WHERE commande = " . $commande . ";";
			db_query($database_name, $sql);
			$sql = "DELETE FROM detail_commandes_pro_test WHERE commande = " . $commande . ";";
			db_query($database_name, $sql);
			
			$profile_id = GetUserProfile();	$user_name=GetUserName();$date_time=date("y-m-d h:m:s");
			$table_name="commandes_test";$record=$_REQUEST["user_id"];
	...
	?>
	<form id="form" name="form" method="post" action="evaluations_client_test.php">
	...
	{$date=dateFrToUs($_POST['date']);$date_f=dateFrToUs($_POST['date']);$vendeur=$_POST['vendeur'];$destination=$_POST['destination'];
		...
		$sql  = "SELECT * ";
		if ($user_login=="admin" or $user_login=="rakia"){
		$sql .= "FROM commandes_test where vendeur='$vendeur' and date_e='$date' ORDER BY date_e;";
		}else
		{
		$sql .= "FROM commandes_test where vendeur='$vendeur' and date_e='$date' and escompte_exercice=0 ORDER BY date_e;";
		}
...
<script type="text/javascript"><!--
	function EditUser(user_id) { document.location = "evaluation_client_test.php?user_id=" + user_id; }
--></script>
...
</tr><tr><? echo "<td><a href=\"evaluation_client_test.php?vendeur=$vendeur&destination=$destination&date=$date_f&user_id=0\">Ajout Commandes des Clients</a></td>";?>
...
<? echo "<td><a href=\"enregistrer_panier_test.php?vendeur=$vendeur&client=$client&date=$date_e&numero=$commande\">$evaluation </a></td>";?>
