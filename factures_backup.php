	...
	if(isset($_REQUEST["action_"])) { 
		...
		switch($_REQUEST["action_"]) {
			...
			case "insert_new_user":
				...
				/*for ($i=1;$i<=5000;$i++){$vendeur="VENTE USINE";
				$sql  = "INSERT INTO factures2017 
...
///////regrouper evaluations
			$sql = "UPDATE factures2017 SET ";$valider_f=1;$v=1;
			$sql .= "numero = '" . $numero_f . "', ";
			$sql .= "valide = '" . $v . "' ";
	...
	while($users_ = fetch_array($users1)) { 
		...
		$sql  = "INSERT INTO detail_factures2016 ( commande,produit,prix_unit,quantite,condit,date_f,facture ) VALUES ( ";
		$sql .= "'" . $commande . "', ";
		$sql .= "'" . $produit . "', ";
		$sql .= "'" . $prix_unit . "', ";
		$sql .= "'" . $quantite . "', ";
...
/// update evaluations
			...
			db_query($database_name, $sql);
			$sql = "DELETE FROM detail_factures2016 WHERE facture = " . $numero_f . ";";
			db_query($database_name, $sql);
			$null=0;
	...
	while($users_ = fetch_array($users1)) { 
		...
		$produit=$users_["produit"];$condit=$users_["condit"];
		$sql  = "INSERT INTO detail_factures2016 ( commande,produit,prix_unit,quantite,condit,date_f,id_origine,facture ) VALUES ( ";
		$sql .= "'" . $commande . "', ";
		$sql .= "'" . $produit . "', ";
		$sql .= "'" . $prix_unit . "', ";
		$sql .= "'" . $quantite . "', ";
	...
	<form id="form" name="form" method="post" action="factures_backup.php">
...
<script type="text/javascript"><!--
	function EditUser(user_id) { document.location = "facture_backup.php?user_id=" + user_id; }
...
<?php $evaluation=$users_["evaluation"]; $client=$users_["client"];$user_id=$users_["id"];

$facture=$users_["id"]+9040;$facture1=$users_["id"]+9040;
echo "<td><a href=\"facture_backup.php?user_id=$user_id&client=$client&facture=$facture&du=$du&au=$au\">$facture1</a></td>";?>
...
<? echo "<td><a href=\"historique_reglements_factures_backup.php?numero=$facture\">R</a></td>";?>
...
$sql  = "SELECT id,date_enc,date_cheque,r_impaye,date_impaye,client,client_tire,client_tire_e,v_banque,numero_cheque,sum(m_cheque) As total_cheque ,
	...
	$sql .= "FROM porte_feuilles_factures where facture_n=$facture Group BY id;";
...
/*while($users_1 = fetch_array($users11)) { 
			...
			<td align="right"><?php $total=$total+$total_cheque;$tc= number_format($total_cheque,2,',',' ');print("<font size=\"0.5\" face=\"Comic sans MS\" color=\"000033\">Ch $tc </font>");  ?></td>
			<td align="right"><?php $total1=$total1+$total_espece;$te= number_format($total_espece,2,',',' '); print("<font size=\"0.5\" face=\"Comic sans MS\" color=\"000033\">Es $te </font>"); ?></td>
			<td align="right"><?php $total11=$total11+$total_effet;$tf= number_format($total_effet,2,',',' '); print("<font size=\"0.5\" face=\"Comic sans MS\" color=\"000033\">Ef $tf </font>"); ?></td>
			<td align="right"><?php $total111=$total111+$total_virement;$tv= number_format($total_virement,2,',',' '); print("<font size=\"0.5\" face=\"Comic sans MS\" color=\"000033\">Vr $tv </font>"); ?></td>
<? }*/?>
...
print("<font size=\"1\" face=\"Comic sans MS\" color=\"000033\">$m </font>");
 ...
 <? echo "<td><a href=\"historique_reglements_factures_backup.php?numero=$facture\"></a></td>";?>
...
<? if ($login=="admin"){echo "<td><a href=\"facture_backup.php?user_id=0&du=$du&au=$au\">Nouvelle Facture</a></td>";}?>
</body>
