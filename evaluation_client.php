<?php
	...
	if($user_id == "0") {
		...
		$client8 = "";
		$client9 = "";
		$client10 = "";$sans_remise6=0;$sans_remise7=0;$sans_remise8=0;$sans_remise9=0;$sans_remise10=0;$escompte=0;$escompte2=0;
		...
		$date9=dateUsToFr($_REQUEST['date']);$date10=dateUsToFr($_REQUEST['date']);$escompte_exercice=0;$encaiss=0;
		$plafond=0;$esc_plafond=0;
		...
		$title = "details";$vendeur=$users_["vendeur"];$id_registre=$users_["id_registre"];$net=$users_["net"];$active=$users_["active"];$reliquat=$users_["reliquat"];
		$ev_pre=$users_["ev_pre"];$bl=$users_["bl"];$bc=$users_["bc"];$piece=$users_["piece"];
		$commande=$users_["commande"];$evaluation=$users_["evaluation"];$client=$users_["client"];	$valider_f=$users_["valider_f"];$escompte_exercice=$users_["escompte_exercice"];	
		$remise10=$users_["remise_10"];$remise2=$users_["remise_2"];$remise3=$users_["remise_3"];$remise4=$users_["remise_4"];
		$sans_remise=$users_["sans_remise"];$date=dateUsToFr($users_["date_e"]);$date=dateUsToFr($users_["date_e"]);$escompte=$users_["escompte"];$escompte2=$users_["escompte2"];
		$plafond=$users_["plafond"];$esc_plafond=$users_["esc_plafond"];$votre_commande=$users_["votre_commande"];$notre_bl=$users_["notre_bl"];
		$numero_facture = $users_["numero_facture"];$date_facture = dateUsToFr($users_["date_facture"]);$en_lettres1 = $users_["en_lettres1"];$en_lettres2 = $users_["en_lettres2"];
		$encaiss = $users_["encaiss"];
		}
...
<script type="text/javascript"><!--
	...
	function DeleteUser() {
		if(window.confirm("<?php ; ?>\n<?php echo "Confirmer la suppression ?"; ?>")) {
			document.location = "evaluations_client.php?action_=delete_user&user_id=<?php echo $_REQUEST["user_id"]; ?>";
		}
	}
<!--
/*
...
<form id="form_user" name="form_user" method="post" action="evaluations_client.php">
	...
	<? if($user_id <> "0") {?>
		...
		<tr><td><?php echo "Date : "; ?></td><td><input onClick="ds_sh(this);" name="date" value="<?php echo $date; ?>" readonly="readonly" style="cursor: text" /></td></tr>
		<tr><td><?php echo "Client : "; ?></td><td><select onkeydown="return liDown(this);" id="client" name="client"><?php echo $client_list; ?></select></td></tr>
		<tr><td><?php echo "Votre commande : "; ?></td><td><input type="text" id="votre_commande" name="votre_commande" style="width:160px" value="<?php echo $votre_commande; ?>"></td>
		<tr><td><?php echo "Notre Numero BL : "; ?></td><td><input type="text" id="notre_bl" name="notre_bl" style="width:160px" value="<?php echo $notre_bl; ?>"></td>
	...
	<tr><td><?php echo "Date Facture: "; ?></td><td><input onClick="ds_sh(this);" name="date_facture" value="<?php echo $date_facture; ?>" readonly="readonly" style="cursor: text" /></td></tr>
		
		<tr><td><?php echo "Numero Facture : "; ?></td><td><input type="text" id="numero_facture" name="numero_facture" style="width:160px" value="<?php echo $numero_facture; ?>"></td>
		<tr><td><?php echo "Montant en lettres : "; ?></td><td><input type="text" id="en_lettres1" name="en_lettres1" style="width:400" value="<?php echo $en_lettres1; ?>"></td>
		
		<tr><td><?php echo "                     "; ?></td><td><input type="text" id="en_lettres2" name="en_lettres2" style="width:400" value="<?php echo $en_lettres2; ?>"></td>
		
		<? if($login == "admin" ) {?>
		<tr><td><?php echo "En compte client"; ?></td><td><input type="text" id="encompte_client" name="encompte_client" style="width:60px" value="<?php echo $encompte_client; ?>"></td>
		</tr>
		<tr><td><?php echo "En compte Avoir"; ?></td><td><input type="text" id="encompte_avoir" name="encompte_avoir" style="width:60px" value="<?php echo $encompte_avoir; ?>"></td>
		<? }?>
		...
		<tr><td><?php echo "1er Escompte %"; ?></td><td><input type="text" id="escompte" name="escompte" style="width:60px" value="<?php echo $escompte; ?>"></td>
		<tr><td><?php echo "2eme Escompte %"; ?></td><td><input type="text" id="escompte2" name="escompte2" style="width:60px" value="<?php echo $escompte2; ?>"></td>
		</tr>
		<tr><td><?php echo "Plafond Escompte"; ?></td><td><input type="text" id="plafond" name="plafond" style="width:60px" value="<?php echo $plafond; ?>"></td>
		<tr><td><?php echo "Escompte Exercice"; ?></td><td><input type="checkbox" id="escompte_exercice" name="escompte_exercice"<?php if($escompte_exercice) { echo " checked"; } ?>></td>
...
<input type="hidden" id="encaiss" name="encaiss" value="<?php echo $encaiss; ?>">
