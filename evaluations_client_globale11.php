<?php
	...
	?>
	<form id="form" name="form" method="post" action="evaluations_client_globale11.php">
	...
	<td><?php echo "Client : "; ?></td><td><select id="client" name="client"><?php echo $profiles_list_vendeur; ?></select>
	...
	{$date=dateFrToUs($_POST['date']);$date_f=dateFrToUs($_POST['date']);$date1=dateFrToUs($_POST['date1']);
		...
		$sql  = "SELECT * ";
		$sql .= "FROM commandes where date_e between '$date' and '$date1' and client='$client' and escompte_exercice=0 ORDER BY date_e;";
		$users = db_query($database_name, $sql);
		}
		
		else
		{$sql  = "SELECT * ";
		$sql .= "FROM commandes where date_e='$date' and escompte_exercice=0 ORDER BY date_e;";
		$users = db_query($database_name, $sql);}
?>
...
$date_ev=$users_["date_e"];$montant_releve=$users_["montant_releve"];$escompte=$users_["escompte"];
...
<td style="text-align:Right"><?php $total_g=$total_g+$netc;echo number_format($netc,2,',',' '); ?></td>
<td style="text-align:Right"><?php echo number_format($escompte,2,',',' ')."%"; ?></td>
