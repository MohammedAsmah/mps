<?php
	...
	if(isset($_REQUEST["action_"]) && $profile_id == 1) { 
		...
		switch($_REQUEST["action_"]) {
			...
			case "insert_new_user":
				...
				$d=dateFrToUs(date("d/m/Y"));$mat="mat2";
				$sql  = "INSERT INTO achats_mat ( produit,type, frs,ref,date,date_ins,prix_achat,ttc,pttt,qte ) VALUES ( ";
				$sql .= "'" . $produit . "', ";
				$sql .= "'" . $mat . "', ";
				$sql .= "'" . $frs . "', ";
				$sql .= "'" . $ref . "', ";
				$sql .= "'" . $date . "', ";
				$sql .= "'" . $d . "', ";
				$sql .= "'" . $prix . "', ";$sql .= "'" . $taux_tva . "', ";$sql .= "'" . $pttt . "', ";
				$sql .= $qte . ");";
			...
			$sql .= "qte = '" . $_REQUEST["qte"] . "', ";$sql .= "pttt = '" . $pttt . "', ";
			$sql .= "date = '" . $date . "', ";
			$sql .= "ttc = '" . $taux_tva . "', ";
			$sql .= "prix_achat = '" . $prix . "' ";
	...
	?>
	<form id="form" name="form" method="post" action="achats_mat2.php">
...
<script type="text/javascript"><!--
	function EditUser(user_id) { document.location = "achat_mat2.php?user_id=" + user_id; }
--></script>
...
<? $t=0;$q=0;while($users_ = fetch_array($users)) {?><tr>
	...
	<td align="right"><?php echo number_format($users_["prix_achat"],2,',',' ');if ($users_["pttt"]){$ttc=$users_["ttc"];}else{$ttc=$users_["ttc"]/1.20;}?></td>
	<td align="right"><?php $t=$t+$ttc;$q=$q+$users_["qte"];
	/*echo number_format($users_["prix_achat"]*$users_["qte"],2,',',' ');*/
