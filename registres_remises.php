...
while($users_1 = fetch_array($users11)) { $id_r=$users_1["id"];$date_enc=$users_1["date"];$banque=$users_1["banque"];
			...
			$sql  = "SELECT * ";
			$sql .= "FROM porte_feuilles where numero_remise=$id_r and m_cheque<>0 and remise=1 ORDER BY id;";
			$users1c = db_query($database_name, $sql);
			$total_controle_cheques=0;
			while($users1_c = fetch_array($users1c)) { 
				
				$total_controle_cheques=$total_controle_cheques+$users1_c["m_cheque"];			
								
			 }
			
			$sql  = "SELECT * ";
			$sql .= "FROM porte_feuilles where numero_remise_e=$id_r and m_effet<>0 and remise_e=1 ORDER BY id;";
			$users1f = db_query($database_name, $sql);
			$total_controle_effets=0;
			while($users1_f = fetch_array($users1f)) { 
				
				$total_controle_effets=$total_controle_effets+$users1_f["m_effet"];			
			...
			echo "<td align=\"right\"><a href=\"registre_remise_details_controle.php?id_registre=$id_r\">".$total_controle_cheques."</a></td>";
			echo "<td align=\"right\"><a href=\"registre_remise_details_e_controle.php?id_registre=$id_r\">".$total_controle_effets."</a></td>";
			$cc=$total_controle_cheques+$total_controle_effets;$mm=$total_g+$total_e_enc+$total_e_escompte;
			if ($cc == $mm){}else{echo "<td align=\"right\">*</td>";}
