...
/*$result=mysql_query('select Code,Name,Price from Products ORDER BY Code',$link);*/
	$sql  = "SELECT * ";
	$sql .= "FROM factures2019 where (date_f between '$date1' and '$date2') and montant>0 ORDER BY id;";
...
while($row = fetch_array($users))
{
	...
	$ville="";$client=$row["client"];$id=$row["id"];$f=$row["numero"];$d=$row["date_f"];$m=$row["montant"];$mm=number_format($m,2,'.',' ');
...
$pdf->SetFont('Arial','B',14);
	...
	$sql  = "SELECT * ";
	$sql .= "FROM detail_factures2019 where facture=$f ORDER BY sans_remise;";
	...
	$pdf->Cell(20,0,$quantite."x".$condit,0,0,'C',1);
	...
	ELSE
	{
			...
			$pdf->Cell(80,0,$t_show,0,0,'C',0);$pdf->SetFont('Arial','B',6);
	...
	$y=$y+3.8;$pdf->SetY($y);$pdf->SetX(145);
	if ($brut_sans<>0){
	...
	$centimes=substr(number_format($chiffres-floor($chiffres), 2, ',', ' '),-2);
	if ($centimes=="00"){$centimes="";}else {$centimes=$centimes." cts";}
	$pdf->SetY(188);$pdf->SetX(15);$texte="Arretee la presente facture a la somme de : ";
 	$pdf->Cell(150,0,$texte,0,0,'L',1);	$y=$y+4;$pdf->SetY(194);$pdf->SetX(15);
 	$pdf->Cell(150,0,$lettres.' '.$centimes,0,0,'L',1);

	$y=$y+9;$pdf->SetY(199);$pdf->SetX(15);$tva=$net/1.20*0.20;
	$tva=number_format($tva, 2, ',', ' ');
 	$pdf->Cell(150,0,'Dont TVA 20 % : '.$tva,0,0,'L',1);
	$l=164;$pdf->SetY($l);$pdf->SetX(112);
	//$pdf->Cell(60,0,'XXXXXXXXXXXXXX   19 000 000.00 DHS ',0,0,'L',1);
