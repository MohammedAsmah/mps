<?php
	...
	?>
	<form id="form" name="form" method="post" action="etat_avoirs_clients_periode.php">
	<table>
	...
	$client=$_POST['client'];$destination=$_POST['destination'];$date_au=$_POST['date2'];$date_du=$_POST['date'];
		...
		{
		$sql  = "SELECT produit,prix_unit,condit,sum(quantite) as quantite ";
		$sql .= "FROM detail_avoirs where client='$client' and date between '$date' and '$date2' group by produit ORDER BY quantite DESC;";
		$users = db_query($database_name, $sql);
		}
		}
...
<span style="font-size:24px"><?php echo "liste Avoirs Du $date_du Au $date_au   -  $client "; ?></span>
...
<?php $produit=$users1_["produit"]; $id=$users1_["id"];
		...
		$sql1 .= "FROM detail_commandes where client='$client' and date between '$date' and '$date2' and produit='$produit' group by produit ORDER BY quantite DESC;";
		...
		<? echo "<a href=\"etat_avoirs_clients_periode_details.php?produit=$produit&date=$date&date2=$date2&client=$client\">$q</a></td>";?>
		<td align="center">
		<? echo "<a href=\"etat_avoirs_clients_periode_details1.php?produit=$produit&date=$date&date2=$date2&client=$client\">$achats</a></td>";?>
...
<? 
		...
		$sql .= "FROM commandes where client='$client' and date_e between '$date' and '$date2' group by vendeur ORDER BY date_e;";
