<?php
	...
	?>
	<form id="form" name="form" method="post" action="evaluations_client_globale.php">
	...
	<td><?php echo "Vendeur : "; ?></td><td><select id="vendeur" name="vendeur"><?php echo $profiles_list_vendeur; ?></select>
	<td><input type="submit" id="action" name="action" value="<?php echo $action; ?>"></td>
	</form>
	
	<? }
	if(isset($_REQUEST["action"]))
	{$date=dateFrToUs($_POST['date']);$date_f=dateFrToUs($_POST['date']);$vendeur=$_POST['vendeur'];$date1=dateFrToUs($_POST['date1']);
		...
		$sql  = "SELECT * ";
		$sql .= "FROM commandes where vendeur='$vendeur' and date_e between '$date' and '$date1' and escompte_exercice=0 ORDER BY date_e;";
		$users = db_query($database_name, $sql);
		}
...
<? ///////////////
		...
		$sql .= "FROM clients WHERE client = '$client' ;";
		$user = db_query($database_name, $sql);
		$user_ = fetch_array($user);$remise10 = $user_["remise10"];$r10=10;
		$remise2 = $user_["remise2"];$ville = $user_["ville"];
