...
xmlhttp.open("GET","choix_famille_frs.php?q="+str,true);
xmlhttp.send();
}
</script>
...
<?
	$client_list = "";
	$sql = "SELECT * FROM  familles_articles_commandes ORDER BY profile_name;";
	$temp = db_query($database_name, $sql);
	while($temp_ = fetch_array($temp)) {
		if($profile_name == $temp_["profile_name"]) { $selected = " selected"; } else { $selected = ""; }
		
		$client_list .= "<OPTION VALUE=\"" . $temp_["profile_name"] . "\"" . $selected . ">";
		$client_list .= $temp_["profile_name"];
		$client_list .= "</OPTION>";
	}
?>
<form>
