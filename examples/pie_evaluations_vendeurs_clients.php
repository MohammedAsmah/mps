...
$graph->title->set("C.A $vendeur PAR CLIENTS $du au $au");
...
$i=1;
	...
	$sql .= "FROM commandes where vendeur='$vendeur' and date_e between '$date1' and '$date2' and evaluation<>'$encours' and id_registre<>0 GROUP BY client order by total_net DESC;";
	$users = db_query($database_name, $sql);
	while($users_ = fetch_array($users)) { 
		if ($i<=10){$pp=$users_['client'].":".number_format($users_['total_net'],2,',',' ');
