<?php
	...
	?>
	<form id="form" name="form" method="post" action="etat_avoirs_clients.php">
	<table>
	...
	$client=$_POST['client'];$destination=$_POST['destination'];$date_au=$_POST['date2'];$date_du=$_POST['date'];
		...
		if ($client==""){
		$sql  = "SELECT * ";
		$sql .= "FROM avoirs where date_e between '$date' and '$date2' ORDER BY date_e;";
		$users = db_query($database_name, $sql);
		}
		else
		{
		$sql  = "SELECT * ";
		$sql .= "FROM avoirs where client='$client' and date_e between '$date' and '$date2' ORDER BY date_e;";
		$users = db_query($database_name, $sql);
		}
		}
		...
		{
		@$client=$_GET['client'];@$date=$_GET['date'];$date_a=$_GET['date'];
		$sql  = "SELECT * ";
		$sql .= "FROM avoirs where client='$client' and date_e between '$date' and '$date2' ORDER BY date_e;";
		$users = db_query($database_name, $sql);}
		
	if(isset($_REQUEST["action_"]))
	{$date=dateFrToUs($_POST['date']);$date_f=dateFrToUs($_POST['date']);$client=$_POST['client'];$date_a=$_POST['date'];
...
<tr>
	...
	<? if ($client==""){?><th><?php echo "Client";?></th><? }?>
</tr>
...
<? if ($client==""){?><td><?php echo $users_["client"]; ?></td><? }?>
<? ///////////////
