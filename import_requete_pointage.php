...
while (!feof($fp))
{
  ...
  $liste[10] = ( isset($liste[10]) ) ? $liste[10] : Null;*/
	...
	$champs2=$liste[2];
	$ch=substr($champs2,0,5);$ch=Trim($ch);$non_inclus=0;$debut=$_POST['debut'];$fin=$_POST['fin'];
	/////////////////////////////////////////////////////////
	/*if ($d=="2011/6/11" and $ch=="C/Out" and $heures<9){$non_inclus=1;}
	if ($d=="2011/6/15" and $ch=="C/In" and $heures>1){$non_inclus=1;}*/
	
	if ($d==$_POST['debut'] and $ch=="C/Out" and $heures<9){$non_inclus=1;}
	if ($d==$_POST['fin'] and $ch=="C/In" and $heures>1){$non_inclus=1;}
	...
	if ($champs0!='')
		...
		,'$date','$heures','$minutes','$lun','$mar','$mer','$jeu','$ven','$sam','$dim','$lun_s','$mar_s','$mer_s','$jeu_s','$ven_s','$sam_s','$dim_s')";  
		*/
		$query = "INSERT INTO pointeuse (id,name,date,statut,audited,date_j,time_long,heures,minutes,non_inclus)
		VALUES('','$champs0','$champs1','$champs2','$ch','$d','$h','$heures','$min','$non_inclus')";  
  		
		$result= mysql_query($query);
  		
		$sql  = "SELECT * ";
		$sql .= "FROM employes where employe='$champs0' and statut=0 ORDER BY employe ;";
		$users2e = db_query($database_name, $sql);$users_2e = fetch_array($users2e);
		$name=$users_2e["employe"];if($name==""){$nt=$nt+1;echo $champs0."<br>";}
