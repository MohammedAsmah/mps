...
set_time_limit(0);
	...
	?>
	<form id="form" name="form" method="post" action="evaluations_clients_escomptes.php">
	...
	{$date=dateFrToUs($_POST['date']);$date_f=dateFrToUs($_POST['date']);$date1=dateFrToUs($_POST['date1']);
		...
		$sql .= "FROM commandes where date_e between '$date' and '$date1' and escompte_exercice=1 ORDER BY date_e,evaluation;";
		$users = db_query($database_name, $sql);
		}
		
		else
		{$sql  = "SELECT * ";$vide="";
		$sql .= "FROM commandes where date_e='$date' and bondesortie<>'$vide' and escompte_exercice=1 ORDER BY date_e,evaluation;";
		$users = db_query($database_name, $sql);}
?>
...
<td><?php echo substr($vendeur,0,8); ?></td>
