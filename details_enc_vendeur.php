...
<? 	
	$date=$_GET['date'];$total=0;$date2=$_GET['date1'];$vendeur=$_GET['vendeur'];
	
	
	$sql  = "SELECT date_remise,id,date_f,date_enc,remise,date_echeance,client,client_tire,client_tire_e,vendeur,numero_cheque,v_banque,facture_n,impaye,
...
while($users_1 = fetch_array($users11)) { 
			...
			$total_diff_prix=$users_1["total_diff_prix"];$total_virement=$users_1["total_virement"];$remise=$users_1["remise"];$date_echeance=$users_1["date_echeance"];
			
			if ($remise==1){$total_cheque_t=$total_cheque_t+$total_cheque;}
			$total_espece=$total_espece;
			$total_espece_t=$total_espece_t+$total_espece;
			if ($date_echeance<=$date1){$total_effet_t=$total_effet_t+$total_effet;}
