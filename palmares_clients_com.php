...
<body style="background:#dfe8ff">
	...
	?>
	<form id="form" name="form" method="post" action="palmares_clients_com.php">
	...
	<tr><td><input type="checkbox" id="avoir" name="avoir"<?php if($avoir) { echo " checked"; } ?>></td><td>avoirs inclus</td>
	...
	*/
	...
	$sql  = "SELECT commande,client,vendeur,date,sum(net) As total,sum(escompte) As total1 ";$comptant="CLIENT AU COMPTANT";
	$sql .= "FROM commandes where date_e between '$date' and '$date1' and evaluation<>'$vide' and evaluation<>'$encours' and client<>'$comptant' and escompte=0 GROUP BY client order by total DESC;";
...
echo "Palmares Clients $du au $au "; ?></span>
...
while($users_ = fetch_array($users)) { $client=$users_["client"];
 $sql12  = "SELECT * ";
		...
		$escompte=$users1_12["escompte"];$sans_escompte=$users1_12["sans_escompte"];$vendeur=$users1_12["vendeur_nom"];
if ($escompte==2 or $sans_escompte==1 ){}else{

$total=$users_["total"];if ($total>$plafond){?><tr><td>
<? print("<font size=\"1\" face=\"Comic sans MS\" color=\"#000033\">$client </font>");?></td>
<td><? print("<font size=\"1\" face=\"Comic sans MS\" color=\"#000033\">$vendeur </font>");?></td>
...
<td align="right"><?php echo number_format($users_["total"]-$net_avoir,2,',',' ');?></td>
<? }?>
<?php 
...
<td align="right"><?php echo number_format($com,2,',',' ')." %";?></td>

<td align="right"><? echo number_format(($users_["total"]-$net_avoir)*$com/100,2,',',' ');?></td>
...
<td align="right"><?php echo number_format($t-$a,2,',',' ');?></td>
<? }?>
<td></td>
