<?php
	...
	<form id="form" name="form" method="post" action="edition_factures_backup_dupliquer.php">
	<td><?php echo "Mois : "; ?></td><td><select id="mois" name="mois"><?php echo $profiles_list_mois; ?></select>
	<td><?php $annee="2023";echo "Annee : "; ?><input type="text" id="annee" name="annee" value="<?php echo $annee; ?>" size="15"></td>
	...
	if ($du>="2018-01-01" and $du<="2018-12-31"){$factures="factures2018";$detail_factures="detail_factures2018";}
			...
			if ($du>="2026-01-01" and $du<="2026-12-31"){$factures="factures2026";$detail_factures="detail_factures2026";}
	$sql  = "SELECT * ";
	$sql .= "FROM ".$factures." where (date_f between '$du' and '$au') and id<2500 and editee=0 and s_controle=0 and montant<>0 ORDER BY id;";
	...
	if ($du>="2018-01-01" and $du<="2018-12-31"){$factures="factures2018";$detail_factures="detail_factures2018";}
			...
			if ($du>="2026-01-01" and $du<="2026-12-31"){$factures="factures2026";$detail_factures="detail_factures2026";}
	$sql  = "SELECT * ";
	$sql .= "FROM ".$factures." where (date_f between '$du' and '$au') and id<2500 ORDER BY id;";
	$users = db_query($database_name, $sql);
	}
	if(isset($_REQUEST["action_2"]))
	{
...
<?php $ca=0;$trouve=0;$nombre=0;while($users_ = fetch_array($users)) { $nombre=$nombre+1;$id_encours=$users_["id"];?><tr>
<?
		...
		$user_ = fetch_array($user);$pr = $user_["produit"];
			...
			$client=$users_["client"];$user_id=$users_["id"];$date_f=$users_["date_f"];$date_ff="2010-12-31";$net=number_format($users_["montant"],0,',','');
			$net_dupliquer=$net;
			///lettres
...
<td align="right" width="150"><?php $ca=$ca+$users_["montant"];$m=number_format($users_["montant"],0,',','');
...
<?php 
	...
	/*$sql  = "SELECT * ";$mois_suivant="2023-11-01";
	$sql .= "FROM ".$factures." where montant='$net_dupliquer' and numero<>'$facture' and date_f<'$mois_suivant' ORDER BY id;";
	$users_dupliquer = db_query($database_name, $sql);$compteur=0;
	while($users_dupliquers = fetch_array($users_dupliquer)) { ?><tr>
	<td><?php $compteur=$compteur+1;$date_dup=$users_dupliquers["date_f"];print("<font size=\"1\" face=\"Comic sans MS\" color=\"000033\">$date_dup </font>"); ?> </td>
	<td><?php $numero_dup=$users_dupliquers["numero"];print("<font size=\"1\" face=\"Comic sans MS\" color=\"000033\">$numero_dup </font>"); ?> </td>
	<? } if ($compteur>0){$trouve = $trouve+1;}?></table></td>
	
	<? */?>
	
	<? $sql  = "SELECT * ";$mois_suivant="2023-11-01";$mois_precedant="2023-02-28";
	$sql .= "FROM ".$factures." where montant='$net_dupliquer' and numero<>'$facture' and date_f<'$mois_suivant' and date_f>'$mois_precedant' ORDER BY id DESC;";
	$users_dupliquer = db_query($database_name, $sql);$compteur=0;
	$users_dupliquers = fetch_array($users_dupliquer) ?>
	<td><?php $compteur=$compteur+1;$date_dup=$users_dupliquers["date_f"];print("<font size=\"1\" face=\"Comic sans MS\" color=\"000033\">$date_dup </font>"); ?> </td>
	<td><?php $numero_dup=$users_dupliquers["numero"];print("<font size=\"1\" face=\"Comic sans MS\" color=\"000033\">$numero_dup </font></td>");
	$id_dup=$users_dupliquers["id"];
	
	
	$sql1  = "SELECT * ";
	$sql1 .= "FROM ".$detail_factures." where facture='$id_dup' and sans_remise=0 ORDER BY produit;";
...
<?php $produit=$users1_["produit"]; $id=$users1_["id"];$m=$users1_["quantite"]*$users1_["prix_unit"]*$users1_["condit"];
		$sub=$users1_["sub"];$quantite=$users1_["quantite"];$condit=$users1_["condit"];$prix_unit=$users1_["prix_unit"];$dupliquer=1;$articles_trouve=$articles_trouve+1;
//
		$sql  = "SELECT * ";
		$sql .= "FROM produits WHERE produit = '$produit' ;";
		$user = db_query($database_name, $sql);
		$user_ = fetch_array($user);$favoris = $user_["favoris"];$pp = $user_["produit"];$dispo_g = $user_["dispo_g"];$matiere = $user_["matiere"];$poids = $user_["poids"];
		
		
		////////////////update
				/*$sql  = "INSERT INTO ".$detail_factures." ( date_ajout,time_ajout,user,facture,date_f, produit,matiere,poids, quantite,prix_unit,dupliquer,condit) VALUES ( ";
				$sql .= "'" . $date_ajout . "', ";
				$sql .= "'" . $time_ajout . "', ";
				$sql .= "'" . $login . "', ";
				$sql .= "'" . $facture . "', ";
...
<?	}
			/*
			if ($articles_trouve>0){$s_controle=1;$dupliquer=1;
			$sql = "UPDATE ".$factures." SET ";
			$sql .= "dupliquer = '" . $dupliquer . "', ";
			$sql .= "s_controle = '" . $s_controle . "' ";
			$sql .= "WHERE id = " . $id_encours . ";";
			db_query($database_name, $sql);}*/
...
<td><?php print("<font size=\"1\" face=\"Comic sans MS\" color=\"000033\">$trouve/$nombre </font>");?></td>
