<?php
	...
	$action="recherche";$vendeur=$_GET["vendeur"];$date=$_GET["date"];
...
<? 	
	...
	$sql .= "FROM porte_feuilles where date_echeance<='$date' and vendeur='$vendeur' and m_effet<>0 and remise_e=0 and eff_f=1 GROUP BY numero_effet;";
	$users11 = db_query($database_name, $sql);
	
?>


<span style="font-size:24px"><?php echo "Porte Feuilles Facturé  : ".$vendeur." au ".dateUsToFr($date); ?></span>
...
while($users_1 = fetch_array($users11)) { 
			$date_enc=$users_1["date_enc"];$date_echeance=$users_1["date_echeance"];
