<?php
	...
	<form id="form" name="form" method="post" action="registres_vendeurs1.php">
...
<? 	if(isset($_REQUEST["action"]))
	...
	$sql  = "SELECT * ";
	$sql .= "FROM registre_vendeurs ORDER BY id;";
...
while($users_1 = fetch_array($users11)) { $id_r=$users_1["id"];$date3=$users_1["date"];$vendeur=$users_1["vendeur"];
			...
			<td><?php echo $users_1["service"]; ?></td>
	...
	while($users_ = fetch_array($users)) { 
		...
		$id=$users_["id"];
			$sql7 = "UPDATE commandes SET ";
			$sql7 .= "bondesortie = '" . $bon . "' ";
			$sql7 .= "WHERE id = " . $id . ";";
			db_query($database_name, $sql7);
		...
		while($users1_ = fetch_array($users1)) { 
			...
			while($users1_ = fetch_array($users1)) { 
			 ...
			 ?>
			<td><?php echo "<a href=\"evaluation_pre.php?observation=$observation&montant=$total_g&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">".$vendeur."</a>"; ?></td>
			...
			<td><?php 
			echo "<a href=\"bon_de_sortie2.php?observation=$observation&montant=$total_g&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">".$bon."</a>"; ?></td>
			<? 
			echo "<td><a href=\"evaluations.php?id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon\">"."Ajout"."</a></td>";
			echo "<td><a href=\"evaluations1.php?id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon\">"."Modif"."</a></td></tr>";
