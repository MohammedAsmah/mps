...
<? 	
	...
	$sql  = "SELECT * ";
	$sql .= "FROM porte_feuilles_factures where client_tire_e='$client_tire_get' and facture_n<>0 and r_impaye_e=1 and m_cheque=0 and m_effet<>0 ORDER BY date_impaye_e;";
...
while($users_1 = fetch_array($users11)) { 
			$date_remise=$users_1["date_remise_e"];
			$client=$users_1["client"];$id=$users_1["id_porte_feuille"];
			$client_tire=$users_1["client_tire_e"];
			$numero_effet=$users_1["numero_effet"];$v_banque=$users_1["v_banque"];
			...
			?><tr>
			<td><?php $n_v= $numero_effet."/".$v_banque;print("<font size=\"2\" face=\"Comic sans MS\" color=\"#000033\">$n_v </font>"); ?></td>
