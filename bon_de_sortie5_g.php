<?php
	...
	$error_message = "";
		...
		$vendeur=$_GET['vendeur'];$date1=dateUsToFr($_GET['date']);$service=$_GET['service'];$livraison=$_GET['livraison'];
	...
	while($users1_ = fetch_array($users1)) { 
		...
		$marron=$users1_["marron"];$gris=$users1_["gris"];$beige=$users1_["beige"];$couleurs=$users1_["couleurs"];
				$prix=$prix*(1-$remise10/100);
				$prix=$prix*(1-$remise2/100);
				$prix=$prix*(1-$remise3/100);
				$sql  = "INSERT INTO bon_de_sortie ( commande, produit, quantite,prix_unit,couleurs,marron,gris,beige,condit ) VALUES ( ";
				$sql .= "'" . $id_registre . "', ";
				$sql .= "'" . $produit . "', ";
				$sql .= "'" . $quantite . "', ";
				$sql .= "'" . $prix . "', ";$sql .= "'" . $couleurs . "', ";$sql .= "'" . $marron . "', ";$sql .= "'" . $gris . "', ";$sql .= "'" . $beige . "', ";
				$sql .= "'" . $condit . "');";
...
$y_axis_initial = 25;$y_axis = 25;$row_height=6;$l=25;$hauteur=4.9;
...
if ($livraison==""){}
else{$pdf->SetY(24);$pdf->SetX(100);$d="Livraison : ".$livraison;
$pdf->Cell(40,10,$d,0,0,'L',1);}
...
$pdf->SetY(37);$pdf->SetX(90);$d="Numero :  ".$bon_sortie;
$pdf->Cell(30,10,$d,0,0,'L',1);

$pdf->SetY(37);$pdf->SetX(145);$d="Montant : ".$montant1;
$pdf->Cell(30,10,$d,0,0,'L',1);
...
$ligne=65;$pdf->Line(3,60,3,65);$pdf->Line(205,60,205,65);
		...
		$sql  = "SELECT couleurs,commande,produit,condit,prix_unit,sum(quantite) as qte,sum(marron) as marron,sum(gris) as gris,sum(beige) as beige ";
	...
	while($users11_ = fetch_array($user1)) { 
		...
		$total=$total+($qte*$condit*$prix);$marron=$users11_["marron"];$gris=$users11_["gris"];$beige=$users11_["beige"];$couleurs=$users11_["couleurs"];
	...
	if ($couleurs==1){
		$pdf->SetX(165);$pdf->Cell(50,6,'M:'.'  '.' B:'.'  '.' G:'.'  ',0,0,'L',1);
	}
	
	$pdf->Line(3,$ligne,3,$ligne+$hauteur);
	$pdf->Line(87,$ligne,87,$ligne+$hauteur);
	$pdf->Line(97,$ligne,97,$ligne+$hauteur);
	$pdf->Line(108,$ligne,108,$ligne+$hauteur);
	$pdf->Line(141,$ligne,141,$ligne+$hauteur);
	$pdf->Line(164,$ligne,164,$ligne+$hauteur);
	...
	$pdf->Line(205,$ligne,205,$ligne+$hauteur);
			...
			$ligne=$ligne+$hauteur;
			//test
			//$pdf->SetX(170);$pdf->Cell(10,6,$ligne,0,0,'L',1);
		 }
		 }
...
$ligne=$ligne+15;$pdf->SetY($ligne);$pdf->SetX(10);$pdf->SetFont('arial','B',14);
...
if ($total-$montant>6000)
	{
	$pdf->SetY($ligne+$hauteur);$pdf->SetX(160);
	$pdf->Cell(50,9,$total,0,0,'L',1);	
	$pdf->line(3, $ligne+$hauteur, 205, $hauteur);
	$pdf->line(5, $ligne+$hauteur, 205, $hauteur);}
