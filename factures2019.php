...
//gets the login
	...
	if(isset($_REQUEST["action_"])) { 
		...
		if($_REQUEST["action_"] != "delete_user") {
			...
			$date =dateFrToUs($_REQUEST["date"]);$date_avoir =dateFrToUs($_REQUEST["date_avoir"]);
		...
		switch($_REQUEST["action_"]) {
			...
			case "insert_new_user":
				...
				/*for ($i=1;$i<=3000;$i++){$vendeur="VENTE USINE";$client="FACTURE EN INSTANCE";$date_f="2025-12-31";
				$sql  = "INSERT INTO factures2025 
...
///////regrouper evaluations
			$sql = "UPDATE factures2025 SET ";$valider_f=1;$v=1;$exec="25";
			$sql .= "numero = '" . $numero_f . "', ";
			$sql .= "exercice = '" . $exec . "', ";
			$sql .= "valide = '" . $v . "' ";
		...
		$vendeur=$user_["vendeur_nom"];
			...
			$sql .= "numero_avoir = '" . $_REQUEST["numero_avoir"] . "', ";
			$sql .= "date_avoir = '" . $date_avoir . "', ";
			$sql .= "montant_avoir = '" . $_REQUEST["montant_avoir"] . "', ";
			$sql .= "nature_avoir = '" . $_REQUEST["nature_avoir"] . "', ";
			$sql .= "ev1 = '" . $ev1 . "', ";
			$sql .= "ev2 = '" . $ev2 . "', ";
			$sql .= "ev3 = '" . $ev3 . "', ";
			$sql .= "ev4 = '" . $ev4 . "', ";
...
/// update evaluations
	...
	$sql  = "SELECT * ";
	$sql .= "FROM factures2025 where  date_f between '$du' and '$au' and id <2000 ORDER BY id;";
	$users = db_query($database_name, $sql);
	}
?>
...
<td align="right" width="150"><?php $ht=$users_["ht"]; print("<font size=\"1\" face=\"Comic sans MS\" color=\"000033\">$ht </font>");?></td>
