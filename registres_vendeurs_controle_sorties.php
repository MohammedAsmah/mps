...
<span style="font-size:30px"><?php echo dateUsToFr($date); ?></span>
...
<?php $compteur1=0;$total_g=0;$total_paquets=0;
while($users_1 = fetch_array($users11)) { $id_r=$users_1["id"];$date3=$users_1["date"];$vendeur=$users_1["vendeur"];$date_fr=dateUsToFr($users_["date_e"]);
			...
			$total_paquets=0;$total_paquets_controle=0;
	...
	while($users_ = fetch_array($users)) { 
		...
		while($users1_ = fetch_array($users1)) { 
			...
			$total=$total+$m;
			$total_paquets=$total_paquets+$users1_["quantite"];
			...
			while($users1_ = fetch_array($users1)) { 
				...
				$total1=$total1+$m;$total_paquets=$total_paquets+$users1_["quantite"];
			 }
	...
	<? 
			 ?><?php 
			  echo "<td><a href=\"entrees_stock_barcode_jp_g_test.php?id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">".$vendeur."</a>"; 
			...
			?>
			 ...
			 <td align="right"><? echo $total_paquets."</td>";
			 
			 
			 $sql1c  = "SELECT * ";
		$sql1c .= "FROM entrees_stock_test where id_registre='$id_r' ORDER BY produit;";
		$users1c = db_query($database_name, $sql1c);$non_favoris=0;
		while($users1_c = fetch_array($users1c)) { 
			$total_paquets_controle=$total_paquets_controle+$users1_c["depot_b"];
			 ...
			 <td align="right"><? echo $total_paquets_controle."</td>";
