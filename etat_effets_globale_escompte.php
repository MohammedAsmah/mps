<?php
	...
	<form id="form" name="form" method="post" action="etat_effets_globale_escompte.php">
...
<? 	 $date=date("Y-m-d");$total=0;
	...
	$sql  = "SELECT date_remise_e,type_remise_effet,date_enc,client,client_tire,client_tire_e,vendeur,numero_effet,v_banque_e,facture_n,impaye,date_echeance,
	...
	$sql .= "FROM porte_feuilles_factures where m_effet<>0 and (date_enc between '$date' and '$date2') group by id ORDER BY date_enc;";
	$users11 = db_query($database_name, $sql);
	
?>


<span style="font-size:24px"><?php echo "Effets Reçus du : ".$_POST['date1']." au ".$_POST['date2']; ?></span>
...
while($users_1 = fetch_array($users11)) { $date_enc=$users_1["date_enc"];$vendeur=$users_1["vendeur"];$impaye=$users_1["impaye"];
			$ref=$users_1["numero_effet"]."/".$users_1["v_banque_e"];$facture_n=$users_1["facture_n"];$date_remise_effet=$users_1["date_remise_e"];
			...
			$total_diff_prix=$users_1["total_diff_prix"];$date_echeance=$users_1["date_echeance"];$type_remise_effet=$users_1["type_remise_effet"];
	...
	$users111 = db_query($database_name, $sql);$user_ = fetch_array($users111);
		$inputation=$user_["inputation"];$patente=$user_["patente"];
			...
			print("<font size=\"1\" face=\"Comic sans MS\" color=\"#000033\">$tchq </font>");
			 ?></td>
			<td><?php $d1= dateUsToFr($date_remise_effet);if ($type_remise_effet=="1"){$escompte="à l'escompte";}else{$escompte="";}
			print("<font size=\"1\" face=\"Comic sans MS\" color=\"#000033\">$d1  </font>"); ?></td> 
			 <td><?php if ($type_remise_effet=="1"){$escompte="à l'escompte";}else{$escompte="";}
			print("<font size=\"1\" face=\"Comic sans MS\" color=\"#000033\">$escompte </font>"); ?></td> 
