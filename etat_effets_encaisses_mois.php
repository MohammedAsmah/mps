<?php
	...
	<form id="form" name="form" method="post" action="etat_effets_encaisses_mois.php">
...
<? 	 $date=date("Y-m-d");$total=0;$date_jour=date("Y-m-d");
	...
	and date_echeance>'$date2' and date_enc between '$date' and '$date2' group by id ORDER BY date_echeance;";
	$users11 = db_query($database_name, $sql);
	
?>


<span style="font-size:24px"><?php echo "Effets Non Echus du : ".$_POST['date1']." au ".$_POST['date2']; ?></span>
...
while($users_1 = fetch_array($users11)) { $date_enc=$users_1["date_enc"];$vendeur=$users_1["vendeur"];$impaye=$users_1["impaye"];
	...
	$users111 = db_query($database_name, $sql);$user_ = fetch_array($users111);
		$inputation=$user_["inputation"];$patente=$user_["patente"];
				...
				$sql  = "SELECT * ";
				$sql .= "FROM factures2019 where numero='$facture_n' ORDER BY id;";
			...
			<td><?php $d2= dateUsToFr($date_enc);print("<font size=\"1\" face=\"Comic sans MS\" color=\"#000033\">$d2 </font>"); ?></td>
