...
//gets the login
	...
	if(isset($_REQUEST["action_"]) && $profile_id == 1) { 
		...
		$remise10_v = $user_["remise10_v"];$remise2_v = $user_["remise2_v"];$remise3_v = $user_["remise3_v"];
		...
		if($_REQUEST["action_"] == "update_user"){	
			...
			$encompte_avoir = $_REQUEST["encompte_avoir"];
					$remise10_v = $user_["remise10_v"];$remise2_v = $user_["remise2_v"];$remise3_v = $user_["remise3_v"];
		if ($remise10_v==0){$remise10=0;}
		if ($remise2_v==0){$remise2=0;}
		if ($remise3_v==0){$remise3=0;}
		...
		switch($_REQUEST["action_"]) {
			...
			case "insert_new_user":
				...
				$sql  = "INSERT INTO commandes ( commande,date_e,client,secteur, type_remise,escompte,vendeur, remise_10,remise_2,remise_3,evaluation,destination,sans_remise ) VALUES ( ";
				$sql .= "'" . $cde . "', ";
				$sql .= "'" . $date . "', ";
				$sql .= "'" . $client . "', ";$sql .= "'" . $secteur . "', ";$sql .= "'" . $type_remise . "', ";$sql .= "'" . $escompte . "', ";
				$sql .= "'" . $vendeur . "', ";
				$sql .= "'" . $remise10 . "', ";
				$sql .= "'" . $remise2 . "', ";
				$sql .= "'" . $remise3 . "', ";
				...
				if ($dir_eval>=100){$dev=$dir_eval;}
				$encours=$mois1." ".$dev;$cde=$dir+1;$encours="encours";
				$sql  = "INSERT INTO commandes ( commande,date_e,client,secteur, vendeur, remise_10,remise_2,remise_3,evaluation,destination,sans_remise ) VALUES ( ";
						$sql .= "'" . $cde . "', ";
						$sql .= "'" . $date1 . "', ";
						$sql .= "'" . $client1 . "', ";$sql .= "'" . $secteur1 . "', ";
						$sql .= "'" . $vendeur . "', ";
						$sql .= "'" . $remise10 . "', ";
						$sql .= "'" . $remise2 . "', ";
						$sql .= "'" . $remise3 . "', ";
	...
	?>
	<form id="form" name="form" method="post" action="evaluations_client_solde.php">
...
<script type="text/javascript"><!--
	function EditUser(user_id) { document.location = "evaluation_client_solde.php?user_id=" + user_id; }
--></script>
...
</tr><tr><? echo "<td><a href=\"evaluation_client_solde.php?vendeur=$vendeur&destination=$destination&date=$date_f&user_id=0\">Ajout Commandes des Clients</a></td>";?>
</tr>
...
while($users_ = fetch_array($users)) { $date_evaluation=$users_["date_e"];$nbjours = round(strtotime($date_jour)-strtotime($date_evaluation))/(60*60*24)-1 ;
if ($nbjours<=3 or $user_login == "najat" or $user_login == "Radia" or $user_login == "rakia" or $user_login == "admin"){?><tr>
...
$vendeur=$users_["vendeur"];$numero=$users_["commande"];$controle=$users_["controle"];$sans_remise=$users_["sans_remise"];$remise10=$users_["remise_10"];
$remise2=$users_["remise_2"];$remise3=$users_["remise_3"];$id=$users_["id"]; $date_en=dateFrToUs($users_["date"]);$ev_pre=$users_["ev_pre"];
echo "<td><a href=\"evaluation_client_solde.php?date=$date_e&user_id=$id\">$evaluation</a></td>";$ref=$users_["vendeur"];?>
...
<? if ($bln<>0)
{echo "<td><a href=\"edition_bl_pdf.php?destination=$destination&numero=$commande&client=$client&bl=$bln&bc=$bcn\"> BL2</a></td>";}?>
