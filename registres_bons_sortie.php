<?php
	...
	<form id="form" name="form" method="post" action="registres_bons_sortie.php">
...
<? 	if(isset($_REQUEST["action"]))
	...
	$sql  = "SELECT * ";
	$sql .= "FROM registre_vendeurs where date='$date' and valide<>0 ORDER BY id;";
	...
	while($users_ = fetch_array($users)) { 
		...
		while($users1_ = fetch_array($users1)) { 
			...
			<td><?php echo $vendeur; ?>
 			...
 			<td><?php 
			echo "<a href=\"bon_de_sortie_magasin.php?observation=$observation&montant=$total_g&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service&login=$login\">".$bon."</a>"; ?></td>
			<td><?php 
			echo "<a href=\"bon_de_sortie_magasin_t.php?observation=$observation&montant=$total_g&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service&login=$login\">".$bon."</a>"; ?></td>
