<?php
	...
	if(isset($_REQUEST["action_"]) && $profile_id == 1) { 
		...
		switch($_REQUEST["action_"]) {
			...
			case "insert_new_user":
				...
				$d=dateFrToUs(date("d/m/Y"));$emb="emb2";
			...
			$sql .= "date = '" . $date . "', ";$sql .= "ttc = '" . $taux_tva . "', ";$sql .= "pttt = '" . $pttt . "', ";
			$sql .= "prix_achat = '" . $prix . "' ";
	...
	?>
	<form id="form" name="form" method="post" action="achats_emb2.php">
	...
	$sql  = "SELECT * ";$emb="emb2";
	$sql .= "FROM achats_mat where type='$emb' and date between '$date' and '$date1' ORDER BY date;";
	...
	$sql  = "SELECT * ";$emb="emb2";
	$sql .= "FROM achats_mat where type='$emb' and date between '$debut_exe' and '$fin_exe' ORDER BY date;";
	$users = db_query($database_name, $sql);
...
<script type="text/javascript"><!--
	function EditUser(user_id) { document.location = "achat_emb2.php?user_id=" + user_id; }
--></script>
...
<span style="font-size:24px"><?php echo "Achats Emballages Divers"; ?></span>
...
<? $t=0;while($users_ = fetch_array($users)) {?><tr>
	...
	<td align="center"><?php echo number_format($users_["prix_achat"],2,',',' ');if ($users_["pttt"]){$ttc=$users_["ttc"];}else{$ttc=$users_["ttc"]/1.20;}?></td>
		<td align="right"><?php $t=$t+$ttc;echo number_format($ttc,2,',',' ');?></td>	
