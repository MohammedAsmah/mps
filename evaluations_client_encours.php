<?php
	...
	if(isset($_REQUEST["action_"]) && $profile_id == 1) { 
		...
		if($_REQUEST["action_"] == "update_user"){	
			...
			$client = $_REQUEST["client"];$vendeur = $_REQUEST["vendeur"];$bc = $_REQUEST["bc"];$piece = $_REQUEST["piece"];
			}
	...
	?>
	<form id="form" name="form" method="post" action="evaluations_client_encours.php">
	<table><td><?php echo "Du: "; ?><input onClick="ds_sh(this);" name="date1" readonly="readonly" style="cursor: text" />
	...
	$date_jour=date("Y-m-d");$encours="encours";
	
	if(isset($_REQUEST["action"]))
	{$date1=dateFrToUs($_POST['date1']);$date_f=dateFrToUs($_POST['date']);$vendeur=$_POST['vendeur'];$date2=dateFrToUs($_POST['date2']);
		
				
		$sql  = "SELECT * ";
		$sql .= "FROM commandes where vendeur='$vendeur' and date_e between '$date1' and '$date2' and evaluation='$encours' ORDER BY date_e;";
		...
		{
		@$vendeur=$_GET['vendeur'];@$date1=$_GET['date1'];@$destination=$_GET['destination'];@$date2=$_GET['date2'];
		$sql  = "SELECT * ";
		$sql .= "FROM commandes where vendeur='$vendeur' and date_e between '$date1' and '$date2' and evaluation='$encours' ORDER BY date_e;";
		$users = db_query($database_name, $sql);}
...
<script type="text/javascript"><!--
	function EditUser(user_id) { document.location = "evaluation_client_encours.php?user_id=" + user_id; }
--></script>
...
<tr>
<td><?php echo dateUsToFr($date1)." Au ".dateUsToFr($date2) ;?></td>
</tr>
...
echo "<td><a href=\"evaluation_client_encours.php?date=$date_e&user_id=$id\">$evaluation</a></td>";$ref=$users_["vendeur"];?>
