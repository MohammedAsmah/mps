...
<?php
	...
	$produit=$_GET['article'];$mois = $date->format('m');
    $sql  = "SELECT id,date,vendeur,sum(montant) As total_net ";
	$sql .= "FROM registre_vendeurs where YEAR(date) ='$annee' GROUP BY YEAR(date) order by total_net DESC;";
