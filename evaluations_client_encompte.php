<?php
	...
	if(isset($_REQUEST["action_"]) && $profile_id == 1) { 
		...
		$date=$_POST['date'];$vendeur=$_POST['vendeur'];$action="recherche";$destination=$_POST['destination'];
			...
			case "update_user":
			$en_compte=$_REQUEST["en_compte"];$en_compte_avoir=$_REQUEST["en_compte_avoir"];
			$sql = "UPDATE commandes SET ";
			$sql .= "en_compte = '" . $en_compte . "', ";
			$sql .= "en_compte_avoir = '" . $en_compte_avoir . "' ";
	...
	?>
	<form id="form" name="form" method="post" action="evaluations_client_encompte.php">
	<table>
	<td><?php echo "Du : "; ?></td><td><input type="text" id="date" name="date" style="width:160px" value="<?php echo $date; ?>"></td>
...
<? $commande=$users_["commande"];$bln=$users_["bl"];$bcn=$users_["bc"];$evaluation=$users_["evaluation"];$client=$users_["client"];$date_e=$users_["date_e"];
$vendeur=$users_["vendeur"];$numero=$users_["commande"];$sans_remise=$users_["sans_remise"];$remise10=$users_["remise_10"];
$remise2=$users_["remise_2"];$net=$users_["net"];$id=$users_["id"];$en_compte=$users_["en_compte"];$en_compte_avoir=$users_["en_compte_avoir"]; $date_en=dateFrToUs($users_["date"]);$ev_pre=$users_["ev_pre"];
echo "<td><a href=\"evaluation_client_encompte.php?date=$date_e&date1=$date1&user_id=$id\">$evaluation</a></td>";$ref=$users_["vendeur"];?>
...
<td style="text-align:Right"><?php echo number_format($total_en_compte,2,',',' '); ?></td>
<td style="text-align:Right"><?php echo number_format($total_en_compte_avoir,2,',',' '); ?></td>
</tr>
