...
<?php
	...
	$produit=$_GET['article'];$mois = $date->format('m');$annee = $date->format('Y');$null="BAIGNOIRE BéBé";
    $sql  = "SELECT date,produit,condit,sum(quantite) As total_quantite ";$mat="MATIERE POLYPROPYLENE";
	$sql .= "FROM detail_commandes where YEAR(date) ='$annee' and escompte_exercice=0 and produit<>'$mat' GROUP BY produit order by produit ;";
