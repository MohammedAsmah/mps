...
?>
	...
	<form id="form" name="form" method="post" action="encaissements_especes_mois_encours.php">
...
<? 	if(isset($_REQUEST["action"]))
	...
	$sql  = "SELECT date_remise,id,date_e,id_commande,date_f,date_enc,client,client_tire,client_tire_e,vendeur,numero_cheque,v_banque,facture_n,impaye,
	sum(montant_e) as total_e,sum(m_cheque) as total_cheque,sum(m_espece) as total_espece, sum(m_effet) as total_effet
	,sum(m_avoir) as total_avoir,sum(m_diff_prix) as total_diff_prix ";
	$sql .= "FROM porte_feuilles where date_enc between '$date' and '$date2' and facture_n=0 
	 Group BY date_enc;";
...
<span style="font-size:24px"><?php echo "Encaissments Espece du mois : ".dateUsToFr($date)." Au ".dateUsToFr($date2)."  ".$vendeur; ?></span>
