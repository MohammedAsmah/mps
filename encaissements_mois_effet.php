<?php
	...
	<form id="form" name="form" method="post" action="encaissements_mois_effet.php">
...
<? 	 $date=date("Y-m-d");$total=0;
	...
	$sql  = "SELECT date_f,date_remise_e,date_enc,client,client_tire,client_tire_e,vendeur,numero_effet,v_banque_e,facture_n,impaye,date_echeance,
	...
	$sql .= "FROM porte_feuilles_factures where m_effet<>0 and (date_remise_e >= '$date') and (date_remise_e <= '$date2') and remise_e=1 group by id ORDER BY date_echeance,numero_effet;";
	$users11 = db_query($database_name, $sql);
	
?>


<span style="font-size:24px"><?php echo "Effets Echus du : ".$_POST['date1']." au ".$_POST['date2']; ?></span>
...
while($users_1 = fetch_array($users11)) { $date_enc=$users_1["date_enc"];$vendeur=$users_1["vendeur"];$impaye=$users_1["impaye"];
			...
			$total_e=$users_1["total_e"];$total_avoir=$users_1["total_avoir"];
			$total_cheque=$users_1["total_cheque"];$total_espece=$users_1["total_espece"];$total_effet=$users_1["total_effet"];
			$total_diff_prix=$users_1["total_diff_prix"];$date_echeance=$users_1["date_echeance"];$date_ff=$users_1["date_f"];
	$sql  = "SELECT * ";
	...
	$users111 = db_query($database_name, $sql);$user_ = fetch_array($users111);
		$inputation=$user_["inputation"];$patente=$user_["patente"];
			...
			if ($facture_n>9039){
				...
				{
				if ($date_f>="2018-01-01"){$factures="factures2018";}else{$factures="factures";}
				$sql  = "SELECT * ";
			...
			<td><?php $df=dateUsToFr($date_ff);print("<font size=\"1\" face=\"Comic sans MS\" color=\"#000033\">$df </font>"); ?></td>
