...
//promotion
	...
	<form id="form" name="form" method="post" action="registres_vendeurs_global.php">
...
<? 	if(isset($_REQUEST["action"]))
	...
	//$sql .= "FROM registre_vendeurs where date='$date' ORDER BY id;";
		$sql .= "FROM registre_vendeurs where date between '$date1' and '$date2' ORDER BY id;";
	$users11 = db_query($database_name, $sql);
?>


<span style="font-size:24px"><?php echo dateUsToFr($date1)." au ".dateUsToFr($date2); ?></span>
...
while($users_1 = fetch_array($users11)) { $id_r=$users_1["id"];$date3=$users_1["date"];$vendeur=$users_1["vendeur"];
			...
			$sql  = "SELECT * ";$v="";
	$sql .= "FROM commandes where id_registre=$id_r and bondesortie='$v' ORDER BY date_e;";
	...
	while($users_ = fetch_array($users)) { 
		...
		while($users1_ = fetch_array($users1)) { 
			...
			if ($valide==1){?><td bgcolor="#33FFCC">
						<? echo "<a href=\"registre_vendeur.php?user_id=$user_id&montant=$ttg\">".$dt."</a>"; ?></td>
	...
	<? 
			...
			echo "<a href=\"bon_de_sortie5.php?observation=$observation&montant=$ttg&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">".$bon."</a>"; ?></td>
			<td><?php 
			echo "<a href=\"bon_de_sortie5_g.php?observation=$observation&montant=$ttg&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">".$bon."</a>"; ?></td>
			
			<?php if ($user_login=="admin"){
			echo "<td><a href=\"bon_de_sortie5_g_v1.php?observation=$observation&montant=$ttg&id_registre=$id_r&date=$date3&vendeur=$vendeur&bon_sortie=$bon&service=$service\">".$bon."</a></td>";} ?>
			...
			echo "<td><a href=\"evaluations1.php?valide=$valide&bon_sortie=$bon&destination=$destination&id_registre=$id_r&date=$date3&vendeur=$vendeur\">"."Modif"."</a></td></tr>";
			}else
			{				
			echo "<td>Validee</td>";
			echo "<td></td>";
