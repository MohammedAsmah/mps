<?php
	...
	if(isset($_REQUEST["action_"]) && $profile_id == 1) { 
		...
		if($_REQUEST["action_"] != "delete_user") {
			...
			$produit = $_REQUEST["produit"];$qte=$_REQUEST["qte"];$prix=$_REQUEST["prix"];$ht=$_REQUEST["ht"];
			$date = dateFrToUs($_REQUEST["date"]);$date1 = $_REQUEST["date"];
			$frs = $_REQUEST["frs"];$ref = $_REQUEST["ref"];$taux_tva = $_REQUEST["taux_tva"];
			if(isset($_REQUEST["pttt"])) { $pttt = 1; } else { $pttt = 0; }
			if(isset($_REQUEST["importation"])) { $importation = 1; } else { $importation = 0; }
		...
		switch($_REQUEST["action_"]) {
			...
			case "insert_new_user":
				...
				$d=dateFrToUs(date("d/m/Y"));$mat="mat";
				$sql  = "INSERT INTO achats_mat ( produit,type, frs,ref,date,date_ins,prix_achat,ttc,ht,pttt,importation,qte ) VALUES ( ";
				$sql .= "'" . $produit . "', ";
				$sql .= "'" . $mat . "', ";
				$sql .= "'" . $frs . "', ";
				$sql .= "'" . $ref . "', ";
			...
			$sql .= "qte = '" . $_REQUEST["qte"] . "', ";$sql .= "pttt = '" . $pttt . "', ";$sql .= "ht = '" . $ht . "', ";
			$sql .= "date = '" . $date . "', ";
			$sql .= "ttc = '" . $taux_tva . "', ";$sql .= "importation = '" . $importation . "', ";
			$sql .= "prix_achat = '" . $prix . "' ";
	...
	?>
	<form id="form" name="form" method="post" action="achats_mat.php">
	...
	$sql  = "SELECT * ";$mat="mat";
	$sql .= "FROM achats_mat where type='$mat' and date between '$date' and '$date1' ORDER BY date;";
	...
	$sql  = "SELECT * ";$mat="mat";
	$sql .= "FROM achats_mat where type='$mat' and date between '$debut_exe' and '$fin_exe' ORDER BY date;";
	$users = db_query($database_name, $sql);
...
<script type="text/javascript"><!--
	function EditUser(user_id) { document.location = "achat_mat.php?user_id=" + user_id; }
--></script>
...
<span style="font-size:24px"><?php echo "Achats Matiere"; ?></span>
...
<? $t=0;$q=0;while($users_ = fetch_array($users)) {?><tr>
	...
	<td align="right"><?php echo number_format($users_["prix_achat"],2,',',' ');
	
	if ($users_["importation"]==1){?>
	
	<td align="right"><?php echo number_format($users_["ht"],2,',',' ');$ttc=$users_["ht"];?></td><?
	
	}else{
	
	if ($users_["pttt"]){$ttc=$users_["ttc"];}else{$ttc=$users_["ttc"]/1.20;}?>
