...
//effet
		...
		, sum(m_effet) as total_effet,sum(m_avoir) as total_avoir,sum(m_diff_prix) as total_diff_prix,sum(m_virement) as total_virement ";
		$sql .= "FROM porte_feuilles_factures where m_effet<>0 and facture_n='$facture' and (date_remise_e between '$date1' and '$date2') and (date_echeance <= '$date2') and remise_e=1 Group BY facture_n;";
		$users11 = db_query($database_name, $sql);
		$t_effet=0;
while($row = fetch_array($users11))
{
	...
	$total_avoir = $row['total_avoir'];
	$total_diff_prix = 0;
	$total_effet = $total_effet+$row['total_effet'];
