...
$pdf->SetY(5);$t_show="DETAIL PORTE FEUILLE CHEQUES ";$pdf->SetX(60);
$pdf->Cell(127,7,$t_show,1,0,'L',1);$pdf->SetY(15);$dt=$_GET['date'];
$pdf->SetX(80);$d="AU : ".dateUstoFr($dt);
...
/*$result=mysql_query('select Code,Name,Price from Products ORDER BY Code',$link);*/
	...
	$sql  = "SELECT id,date_enc,date_cheque,date_echeance,client,client_tire_e,v_banque,numero_cheque,sum(m_cheque) As total_cheque ";
	$sql .= "FROM porte_feuilles where m_cheque<>0 and remise=0 and chq_f=1 and date_cheque<='$dt' GROUP BY numero_cheque order by date_cheque;";
...
while($row = fetch_array($users))
{
	...
	if ($i == $max)
	{
		...
		$i = 0;
			...
			$numero_cheque=$row['numero_cheque'];$client=$row['client'];
	...
	$pdf->SetX(15);$input="";$date=dateUsToFr($date_cheque);
