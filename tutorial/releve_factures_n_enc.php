...
$facture=$row["id"]+9040;
	...
	$client=$row["client"];$id=$row["id"];$f=$row["numero"];$d=$row["date_f"];$client_se=Trim($client);$datefacture=$row["date_f"];
...
///cheques		
		...
		$sql .= " or (m_effet<>0 and facture_n='$facture' and date_f='$datefacture' and (date_remise_e between '$date1' and '$date2') and (date_echeance <= '$date2') and remise_e=1 )";
		//m_effet<>0 and facture_n='$facture' and date_f='$datefacture' and (date_remise_e between '$date1' and '$date2') and (date_echeance <= '$date2') and remise_e=1
...
/*
		...
		, sum(m_effet) as total_effet,sum(m_avoir) as total_avoir,sum(m_diff_prix) as total_diff_prix,sum(m_virement) as total_virement ";
		$sql .= "FROM porte_feuilles_factures where m_effet<>0 and facture_n='$facture' and date_f='$datefacture' and (date_remise_e between '$date1' and '$date2') and (date_echeance <= '$date2') and remise_e=1 Group BY id;";
		$users11 = db_query($database_name, $sql);
		$t_effet=0;
while($row = fetch_array($users11))
{
...
//effet nechus
		$sql  = "SELECT remise,numero_cheque,facture_n,client,client_tire,v_banque,sum(montant_e) as total_e,sum(m_cheque) as total_cheque,sum(m_espece) as total_espece
		, sum(m_effet) as total_effet,sum(m_avoir) as total_avoir,sum(m_diff_prix) as total_diff_prix,sum(m_virement) as total_virement ";
		$sql .= "FROM porte_feuilles_factures where m_effet<>0 and facture_n='$facture' and date_f='$datefacture' and (date_echeance > '$date2') and (date_enc <='$date2') and id_registre_regl<>0 Group BY facture_n;";
