...
$y_axis_initial = 25;$y_axis = 25;$row_height=6;
	...
	$a="A";$date2=$_GET['date2'];$vendeur_r=$_GET['vendeur'];
...
$pdf->SetY(15);$t_show="ENCAISSEMENTS DU MOIS  : ".dateUsToFr($date1)."   ".dateUsToFr($date2)."  ".$vendeur_r;
...
$y_axis = $y_axis + $row_height;$dd="2009-09-30";$hors="hors compte";$hors1="hors";$vide="";$fff="13641";
...
/*$result=mysql_query('select Code,Name,Price from Products ORDER BY Code',$link);*/
		...
		client_tire,v_banque,date_cheque,remise,facture_n,vendeur,date_facture,numero_facture,date_remise_e,
		...
		(date_remise between '$date1' and '$date2' and facture_n<>0 and remise=1 and numero_cheque_imp='$vide' and m_effet=0 and m_cheque<>0) or
		 (facture_n<>0 and m_effet<>0 and m_cheque=0 and remise_e=1 and date_remise_e>='$date1' and date_remise_e<='$date2' and numero_cheque_imp='$vide')   
		
		or (date_virement between '$date1' and '$date2' and facture_n<>0 and m_virement<>0 and id_registre_regl<>0 and m_effet=0 and m_cheque=0)
		or (date_enc between '$date1' and '$date2' and facture_n<>0 and m_espece<>0 and m_cheque=0 and m_effet=0 and m_virement=0) 
...
while($row = fetch_array($users11)){
	...
	$client = $row['client'];$client_tire=$row['client_tire'];$date_remise=dateUsToFr($row['date_remise']);$date_remise_e=dateUsToFr($row['date_remise_e']);
	$date_remise_c = $row['date_remise'];$date_e = $row['date_e'];$vendeur = $row['vendeur'];$date_echeance = $row['date_echeance'];
	$banque=$row['v_banque'];$facture_n=$row['facture_n'];$date_enc = $row['date_enc'];$date_enc1 = dateUsToFr($row['date_enc']);
	$numero_cheque=$row['numero_cheque'];$date_virement = $row['date_virement'];$date_v = dateUsToFr($row['date_virement']);$date_f=$row['date_f'];
	$date_facture=dateUstoFr($row['date_facture']);$numero_facture=$row['numero_facture'];
	...
	$numero_facture=$facture_n;
	}else
	{
	if ($date_f>="2018-01-01"){$factures="factures2018";$exe2=$zero.$facture_n."/18";}else{$factures="factures";$exe2=$zero.$facture_n."/17";}
	$sql  = "SELECT * ";
	$sql .= "FROM ".$factures." where numero='$facture_n' ORDER BY id;";
	$users1 = db_query($database_name, $sql);$row1 = fetch_array($users1);$dt=dateUsToFr($row1["date_f"]);
	
	
	$exercice=$row1["exercice"];$numero_facture=$exe2;
	}
	*/
	...
	$users111 = db_query($database_name, $sql);$user_ = fetch_array($users111);
		$inputation=$user_["inputation"];$patente=$user_["patente"];$client1=$client."/".$user_["vendeur_nom"];
	
	
	if ($vendeur_r==""){
	//if ($user_["vendeur_nom"]=="RAKIA ABDELKADER"){
	
	if ($row['date_facture']<$date1){
	
	$pdf->SetY($y_axis);
	$pdf->SetX(5);$input="";
	$cheque = $row['total_cheque'];
	...
	$total_effet=$total_effet+$effet;
	if ($date_remise=="00/00/0000"){$date_remise=$date_remise_e;}
	...
	$pdf->Cell(23,6,$date_facture,1,0,'L',1);
	$pdf->Cell(16,6,$numero_facture,1,0,'L',1);
	...
	if ($user_["vendeur_nom"]==$vendeur_r){
