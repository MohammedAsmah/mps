...
//Set Row Height
$row_height = 7;$t_espece=0;$t_av=0;
...
while($row = fetch_array($users11))
{
	...
	$t_av=$t_av+$total_avoir;
	$t_chq=$t_chq+$total_cheque;
	$t_eff=$t_eff+$total_effet;
	$t_vir=$t_vir+$total_virement;
	...
	$pdf->Cell(30,6,number_format($v,2,',',' '),1,0,'R',1);
...
$pdf->SetX(100);
	$pdf->Cell(30,6,'REPPORT',1,0,'L',1);
	$pdf->Cell(25,6,number_format($t_av,2,',',' '),1,0,'R',1);$pdf->Cell(25,6,number_format(0,2,',',' '),1,0,'R',1);
	$pdf->Cell(30,6,$t_espe,1,0,'R',1);
	$pdf->Cell(25,6,number_format($t_chq,2,',',' '),1,0,'R',1);
	$pdf->Cell(25,6,number_format($t_eff,2,',',' '),1,0,'R',1);
	$pdf->Cell(25,6,number_format($t_vir,2,',',' '),1,0,'R',1);
	$y_axis = $y_axis + $row_height;
		}	
	...
	$pdf->SetX(130);
	$pdf->Cell(25,6,number_format($t_av,2,',',' '),1,0,'R',1);
	$pdf->Cell(25,6,number_format(0,2,',',' '),1,0,'R',1);
	$pdf->Cell(30,6,$t_espe,1,0,'R',1);
	$pdf->Cell(25,6,number_format($t_chq,2,',',' '),1,0,'R',1);
	$pdf->Cell(25,6,number_format($t_eff,2,',',' '),1,0,'R',1);
	$pdf->Cell(25,6,number_format($t_vir,2,',',' '),1,0,'R',1);
	...
	$pdf->SetX(60);
	$pdf->Cell(120,6,$libelle1,1,0,'L',1);
	$pdf->Cell(30,6,$montant1,1,0,'R',1);
	$i = $i + 1;
	if ($i >= $max)
	{
		$pdf->AddPage();
		$i=0;$i=0;$y_axis_initial = 25;$y_axis = 25;$row_height=6;$l=25;
		}
	} 	
	...
	$pdf->Ln();$y_axis=$y_axis+5;
	$pdf->SetX(60);
	$pdf->Cell(120,6,$libelle2,1,0,'L',1);
	$pdf->Cell(30,6,$montant2,1,0,'R',1);
	$i = $i + 1;
	if ($i >= $max)
	{
		$pdf->AddPage();
	...
	$pdf->Ln();$y_axis=$y_axis+5;
	$pdf->SetX(60);
	$pdf->Cell(120,6,$libelle3,1,0,'L',1);
	$pdf->Cell(30,6,$montant3,1,0,'R',1);
	$i = $i + 1;
	if ($i >= $max)
	{
		$pdf->AddPage();
	...
	$y_axis=$y_axis+35;$pdf->SetY($y_axis);
	$pdf->Ln();
	$pdf->SetX(120);
