...
$banque=$_GET['banque'];$exe1=dateFrToUs($_GET['date']);$date=$_GET['date'];$du=$_GET['du'];$au=$_GET['au'];$du_fr=dateUsToFr($du);$au_fr=dateUsToFr($au);
		...
		$exe="2008-01-01";
			$sql  = "SELECT * ";
			$sql .= "FROM journal_banques where caisse='$banque' and date>='$exe' and erreur=0 ORDER BY date,id;";
...
$pdf->SetY(1);$t_show="Tresorerie Du : ".$du_fr."  Au ".$au_fr;$pdf->SetX(50);
$pdf->Cell(97,6,$t_show,1,0,'L',1);
$pdf->SetFont('Arial','B',12);
$pdf->SetY($y_axis_initial);
$pdf->SetX(3);
$pdf->Cell(25,8,'Date',1,0,'M',1);
$pdf->Cell(150,8,'Libelle',1,0,'M',1);
$pdf->Cell(25,8,'Imputation',1,0,'M',1);
...
$pdf->Cell(27,8,'Solde',1,0,'M',1);
	...
	if ($row["date"]>=$du and $row["date"]<=$au){
	$pdf->SetY($y_axis);$vide="";
	$pdf->SetX(3);$input="";$datefr=dateUsToFr($row["date"]);
	$pdf->Cell(25,6,$datefr,1,0,'L',1);
	$pdf->Cell(150,6,$row["libelle"],1,0,'L',1);
	$pdf->Cell(25,6,$row["imputation"],1,0,'C',1);
	...
	$pdf->Cell(27,6,number_format($s1,2,',',' '),1,0,'R',1);
	//Go to next row
