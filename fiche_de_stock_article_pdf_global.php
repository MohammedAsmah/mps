...
require('fpdf.php');
	...
	$qte_tige1=0;
		...
		// gets user infos
		$sql  = "SELECT * ";$dispo=1;$ty='accessoire';
		$sql .= "FROM produits WHERE type<>'$ty' and dispo = " . $dispo . ";";
		...
		$matiere=$user_["matiere"];$qte_matiere=$user_["qte_matiere"];$etiquette=$user_["etiquette"];$qte_etiquette=$user_["qte_etiquette"];
				...
				$ref="Stock initial au 01/01";$date_ini="2014-01-01";$t1=1;
			...
			$sql1  = "SELECT * ";$du="2014-01-01";$au=dateFrToUs("31/12/2014");
			...
			$sql1  = "SELECT * ";$qte_vendu=0;$du="2014-01-01";$au=dateFrToUs("31/12/2014");
...
// gets user infos
		$sql  = "SELECT * ";$dispo=1;
		//$sql .= "FROM produits WHERE type<>'$ty' and dispo = " . $dispo . ";";
		$sql .= "FROM produits WHERE type<>'$ty' and dispo = '$dispo' ORDER BY produit;";
		
		$user1 = db_query($database_name, $sql); 
		
		while($user_1 = fetch_array($user1)) { 
		$produit = $user_1["produit"];$stock=0;$stock_ini_exe = $user_1["stock_ini_exe"];
	$sql2  = "SELECT * ";
	$sql2 .= "FROM fiche_stock_f where produit='$produit' ORDER BY date,type;";
		$users2 = db_query($database_name, $sql2);$e=0;$s=0;
			while($users2_ = fetch_array($users2)) { 
			$date=dateUsToFr($users2_["date"]);$ref=$users2_["ref"];$entree=$users2_["entree"];$sortie=$users2_["sortie"];
			$type=$users2_["type"];$prix_unit=$users2_["prix_unit"];
			$e=$e+$entree;$s=$s+$sortie;$stock=$e-$s;		
			}

			
			if ($stock_ini_exe>0 or $e>0 or $s>0){			
...
$pdf->SetY(15);$pdf->SetX(12);$d="  Date  ";
$pdf->Cell(15,9,$d,0,0,'L',1);

$pdf->SetX(35);$d="DESIGNATION ";
$pdf->Cell(80,9,$d,0,0,'L',1);

$pdf->SetY(15);$pdf->SetX(125);$d="ENTREE ";
$pdf->Cell(30,9,$d,0,0,'L',1);
$pdf->SetY(15);$pdf->SetX(150);$d=" SORTIE ";
$pdf->Cell(30,9,$d,0,0,'L',1);
...
$pdf->SetFont('arial','',12);	
		...
		$sql1  = "SELECT * ";$du="2014-01-01";$au=dateFrToUs(date("d/m/y"));
	...
	$pdf->Cell(20,6,$date,0,0,'L',1);$pdf->SetX(35);
	$pdf->Cell(80,6,$ref,0,0,'L',1);$pdf->SetX(117);
	if ($type==1){
	$pdf->Cell(20,6,$entree,0,0,'R',1);$pdf->SetX(147);
	$pdf->Cell(20,6,'',0,0,'R',1);$pdf->SetX(177);
	}
	else
	{
	$pdf->Cell(20,6,'',0,0,'R',1);$pdf->SetX(147);
	$pdf->Cell(20,6,$sortie,0,0,'R',1);$pdf->SetX(177);
	}
	$pdf->Cell(20,6,$stock,0,0,'R',1);
	
	
	$pdf->Line(3,$ligne,3,$ligne+5);
	$pdf->Line(33,$ligne,33,$ligne+5);
	...
	$pdf->Line(33,$ligne,33,$ligne+4);
	$pdf->Line(115,$ligne,115,$ligne+4);
	$pdf->Line(144,$ligne,144,$ligne+4);
	$pdf->Line(174,$ligne,174,$ligne+4);
	$pdf->Line(3,$ligne+4,205,$ligne+4);
	
	//CUMULS
	$pdf->SetY($ligne+5);
	$pdf->SetX(90);$pdf->Cell(20,6,'Cumuls a reporter',0,0,'R',1);
	$pdf->SetX(112);
	$pdf->Cell(20,6,$e,0,0,'R',1);
	$pdf->SetX(142);
	$pdf->Cell(20,6,$s,0,0,'R',1);
	$pdf->SetX(172);
	$pdf->Cell(20,6,$stock,0,0,'R',1);
	$pdf->Line(115,$ligne+5,115,$ligne+10);
	$pdf->Line(144,$ligne+5,144,$ligne+10);
	$pdf->Line(174,$ligne+5,174,$ligne+10);	 
	$pdf->Line(205,$ligne+5,205,$ligne+10);	
	$pdf->Line(115,$ligne+10,205,$ligne+10);	 
