<?php
	...
	<form id="form" name="form" method="post" action="effets_echeances_hors_exercices.php">
...
<? 	 $date=date("Y-m-d");$total=0;
	...
	$sql .= "FROM porte_feuilles where date_enc between '$date1' and '$date2' and m_effet<>0 and date_echeance >'$date2' group by id ORDER BY date_echeance;";
	$users11 = db_query($database_name, $sql);
	
	/*$sql  = "SELECT date_remise,date_remise_e,date_enc,client,client_tire,client_tire_e,vendeur,numero_effet,v_banque_e,facture_n,impaye,date_echeance,
	sum(montant_e) as total_e,sum(m_cheque) as total_cheque,sum(m_espece) as total_espece, sum(m_effet) as total_effet
	,sum(m_avoir) as total_avoir,sum(m_diff_prix) as total_diff_prix ";
	$sql .= "FROM porte_feuilles where date_echeance between '$date1' and '$date2' and m_effet<>0 and 
	date_remise_e >'$date2' group by id ORDER BY date_echeance;";
	$users11 = db_query($database_name, $sql);*/
?>


<span style="font-size:24px"><?php echo " Effets Date remise Hors exercice du : ".$_POST['date1']." au ".$_POST['date2']; ?></span>
...
while($users_1 = fetch_array($users11)) { $date_enc=$users_1["date_enc"];$vendeur=$users_1["vendeur"];$impaye=$users_1["impaye"];
			...
			$total_diff_prix=$users_1["total_diff_prix"];$date_echeance=$users_1["date_echeance"];$date_r=$users_1["date_remise_e"];
	$sql  = "SELECT * ";
	...
	$users111 = db_query($database_name, $sql);$user_ = fetch_array($users111);
		$inputation=$user_["inputation"];$patente=$user_["patente"];
			...
			<td><?php $de=dateUsToFr($date_enc);print("<font size=\"1\" face=\"Comic sans MS\" color=\"#000033\">$de </font>"); ?></td>
