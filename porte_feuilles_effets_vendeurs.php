...
<?php if($error_message != "") { echo $error_message . "<p>"; } ?><p>
	...
	?>
	<form id="form" name="form" method="post" action="porte_feuilles_effets_vendeurs.php">
	...
	$sql  = "SELECT id,date_enc,date_cheque,date_echeance,client,client_tire,v_banque,vendeur,numero_cheque,sum(m_effet) As total_cheque ";
	$sql .= "FROM porte_feuilles where date_echeance<='$date' and m_effet<>0 and remise_e=0 and eff_f=1 GROUP BY vendeur;";
...
while($users_1 = fetch_array($users11)) { 
			...
			$client=$users_1["client"];$client_tire=$users_1["client_tire"];$vendeur=$users_1["vendeur"];
			...
			<? echo "<tr><td><a href=\"porte_feuilles_effet_details1v.php?vendeur=$vendeur&date=$date\">$vendeur</a></td>";?>
			<td align="right"><? $total_g=$total_g+$total_cheque;$tc=number_format($total_cheque,2,',',' ');
			echo "<a href=\"porte_feuilles_effet_details1v.php?vendeur=$vendeur\">$tc</a></td>";?>
...
<tr><? echo "<td><a href=\"\\mps\\tutorial\\porte_feuilles_effets_vendeurs.php?date=$date\">Imprimer</a></td>";?>
<tr><? //echo "<td><a href=\"\\mps\\tutorial\\porte_feuilles_effets_details_vendeurs.php\">Imprimer detail</a></td>";?>
