<?php
	...
	<form id="form" name="form" method="post" action="porte_feuilles_controle_com.php">
...
<? 	if(isset($_REQUEST["action"]))
	...
	$sql .= "FROM porte_feuilles where vendeur='$vendeur' and (date_enc between '$date' and '$date2') Group BY id;";
	$users11 = db_query($database_name, $sql);
...
while($users_1 = fetch_array($users11)) { $date_enc=dateUsToFr($users_1["date_enc"]);$vendeur=$users_1["vendeur"];$impaye=$users_1["impaye"];
			...
			$date_e=$user_r["date_e"];$evaluation=$user_r["evaluation"];}
			else
			{
			$sql  = "SELECT * ";
			$sql .= "FROM factures where id=$id_commande GROUP BY id;";
			$users33 = db_query($database_name, $sql);$user_r3 = fetch_array($users33);
			$date_e=$user_r3["date_f"];$evaluation="";}*/
			...
			if ($date_e>=$date and $date_e<=$date2){
			...
			<td><?php $date_ee=dateUsToFr($date_e);print("<font size=\"2\" face=\"Comic sans MS\" color=\"#000033\">$date_ee </font>"); ?></td>
