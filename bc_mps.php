<?php
	...
	if(isset($_REQUEST["action_"]) && $profile_id == 1) { 
		...
		$date_f = dateFrToUs($_REQUEST["date"]);$bb = $_REQUEST["bb"];$observation = $_REQUEST["observation"];
			...
			if(isset($_REQUEST["ttc"])) { $ttc = 1; } else { $ttc = 0; }
		$sql  = "SELECT * ";
		...
		switch($_REQUEST["action_"]) {
			...
			case "insert_new_user":
				...
				$sql  = "INSERT INTO commandes_frs ( user,date_time,date_e,client,ville,fax, vendeur, remise_10,remise_2,remise_3,evaluation,ttc,bb,observation,destination,sans_remise ) VALUES ( ";
				$sql .= "'" . $user_name . "', ";
				$sql .= "'" . $date_time . "', ";
				$sql .= "'" . $date . "', ";
				$sql .= "'" . $client . "', ";
		...
		$date=$_POST['date'];$vendeur=$_POST['vendeur'];$action="recherche";$destination=$_POST['destination'];
			...
			$date_cloture = dateFrToUs($_REQUEST["date_cloture"]);$obs_cloture = $_REQUEST["obs_cloture"];			
			...
			$sql .= "piece = '" . $piece . "', ";
			$sql .= "bl = '" . $bl . "', ";*/
			$sql .= "bb = '" . $bb . "', ";$sql .= "observation = '" . $observation . "', ";
			...
			db_query($database_name, $sql);
				...
				$table_name="commandes_frs";$record=$_REQUEST["user_id"];
	...
	<table><td><?php echo "Date: "; ?><input onClick="ds_sh(this);" name="date" readonly="readonly" style="cursor: text" /></td>
	...
	{$date=dateFrToUs($_POST['date']);$date_f=dateFrToUs($_POST['date']);$vendeur=$_POST['vendeur'];
		...
		/*$sql  = "SELECT * ";
		$sql .= "FROM commandes_frs ORDER BY date_e;";
		$users = db_query($database_name, $sql);*/
...
<? $commande=$users_["commande"];$bln=$users_["bl"];$bcn=$users_["bc"];$evaluation=$users_["evaluation"];$client=$users_["client"];
...
<td><?php echo $date_cloture; ?></td>

<? /*$sql = "UPDATE commandes_frs SET ";$dd="0000-00-00";
			
			$sql .= "date_e = '" . $date_en . "' ";
				
			$sql .= "WHERE date_e='$dd' and id = " . $id . ";";
			db_query($database_name, $sql);*/?>
