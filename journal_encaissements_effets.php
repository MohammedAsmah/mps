<?php
	...
	<form id="form" name="form" method="post" action="journal_encaissements_effets.php">
...
<? $e="ETAT DES ENTREES EFFETS : ";print("<font size=\"6\" face=\"Comic sans MS\" color=\"000033\">$e </font>"); ?></td></span>
</table>
<table border="0" class="table2" bordercolorlight="#FFFBF0" bordercolor="#FFFBF0">
...
<?
	...
	$sql  = "SELECT facture_n,v_banque,numero_effet,client,client_tire,client_tire_e,sum(montant_e) as total_e,sum(m_espece) as total_espece, sum(m_effet) as total_effet
	,sum(m_avoir) as total_avoir,sum(m_diff_prix) as total_diff_prix ";
	$sql .= "FROM porte_feuilles where id_registre_regl='$id_r' and m_effet<>0 and eff_f=1 Group BY numero_effet;";
	...
	while($users_1 = fetch_array($users11)) { 
			...
			$total_diff_prix=$users_1["total_diff_prix"];$numero_cheque=$users_1["numero_effet"];$v_banque=$users_1["v_banque"];
			$total_c1=$total_c1+$total_effet;
...
<tr>
<td align="center" bordercolor="#FFFBF0"><? $link="<a href=\"\\mps\\tutorial\\journal_encaissement_tableau_effets.php?tableau=$id_r&vendeur=$vendeur&service=$service\">".$tableau." - ".$date_t."</a>";?>
...
<?php $compteur1=0;$total_g=0;$total_e=0;$total_c=0;$total_t=0;
	$sql  = "SELECT facture_n,v_banque,numero_cheque,date_echeance,client,client_tire,client_tire_e,sum(montant_e) as total_e,sum(m_cheque) as total_cheque,sum(m_espece) as total_espece, sum(m_effet) as total_effet
	,sum(m_avoir) as total_avoir,sum(m_diff_prix) as total_diff_prix ";
	$sql .= "FROM porte_feuilles where id_registre_regl='$id_r' and m_effet<>0 and eff_f=1 Group BY numero_effet;";
	$users11 = db_query($database_name, $sql);
while($users_12 = fetch_array($users11)) { 
			...
			$total_diff_prix=$users_12["total_diff_prix"];$numero_cheque=$users_12["numero_cheque"];
			$v_banque=$users_12["v_banque"];$date_echeance=dateUsTofr($users_12["date_echeance"]);?>
			<tr>
			<td bordercolor="#FFFBF0"><?php $clt=$client." / ".$client_tire_e." / ".$date_echeance; print("<font size=\"2\" face=\"Comic sans MS\" color=\"000033\">$clt </font>");
			?></td>
			<td align="right" bordercolor="#FFFBF0" width="100"><?php $total_cc=$total_cc+$total_effet;
			$total_c=$total_c+$total_effet;$tt=number_format($total_effet,2,',',' ');
