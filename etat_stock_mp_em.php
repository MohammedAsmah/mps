<?php
	...
	$action="recherche";
	$type_list = "";
	$sql = "SELECT * FROM  famille_emballages ORDER BY type;";
	$temp = db_query($database_name, $sql);
	while($temp_ = fetch_array($temp)) {
		if($profile_name == $temp_["type"]) { $selected = " selected"; } else { $selected = ""; }
		
		$type_list .= "<OPTION VALUE=\"" . $temp_["type"] . "\"" . $selected . ">";
		$type_list .= $temp_["type"];
		$type_list .= "</OPTION>";
	}
	...
	<form id="form" name="form" method="post" action="etat_stock_mp_em.php">
	<TR><td><?php echo "Famille : "; ?></td><td><select id="type" name="type"><?php echo $type_list; ?></select></td></TR>
	...
	$type=$_POST['type'];$au=dateFrToUs($_POST['au']);$aufr=$_POST['au'];
	$sql  = "SELECT * ";$t1="mp";
	$sql .= "FROM matieres_premieres where type='$type' ORDER BY produit;";
	...
	$sql .= "FROM matieres_premieres where type<>'$mr' and type<>'$mp' ORDER BY type,produit;";
	$users = db_query($database_name, $sql);
	}
?>
...
<span style="font-size:24px"><?php echo "STOCK MATIERES PREMIERES $type au $aufr"; ?></span>
...
$p=$users_["produit"];$type=$users_["type"];
