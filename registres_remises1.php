<?php
	...
	// recherche contrat			
				...
				?><table><?
				while($users_ = fetch_array($users)) { $id=$users_["id"];$m_cheque=$users_["m_cheque"];$numero_cheque=$users_["numero_cheque"];
			...
			db_query($database_name, $sql);$impaye=1;$id_registre=$_REQUEST["user_id"];
				...
				while($users_ = fetch_array($users)) { $id=$users_["id"];$m_cheque=$users_["m_cheque"];$numero_cheque=$users_["numero_cheque"];
					...
					$sql .= "date_impaye = '" . $d_impaye1 . "', ";
					$sql .= "r_impaye = '" . $r_impaye1 . "' ";
					$sql .= "WHERE numero_effet = '$ref1' ;";
					db_query($database_name, $sql);}
					else {$sql = "UPDATE porte_feuilles SET ";$d_impaye="0000-00-00";$r=0;
					$sql .= "date_impaye = '" . $d_impaye . "', ";
					$sql .= "r_impaye = '" . $r . "' ";
					$sql .= "WHERE numero_effet = '$ref1' ;";
					db_query($database_name, $sql);}*/
					...
					/*if ($r_impaye2==1){
					$sql = "UPDATE porte_feuilles SET ";$remise=1;
					$sql .= "date_impaye = '" . $d_impaye2 . "', ";
					$sql .= "r_impaye = '" . $r_impaye2 . "' ";
					$sql .= "WHERE numero_effet = '$ref2' ;";
					db_query($database_name, $sql);}
					else {
					$sql = "UPDATE porte_feuilles SET ";$d_impaye="0000-00-00";$r=0;
					$sql .= "date_impaye = '" . $d_impaye . "', ";
					$sql .= "r_impaye = '" . $r . "' ";
					$sql .= "WHERE numero_effet = '$ref2' ;";
					db_query($database_name, $sql);}*/
	...
	<form id="form" name="form" method="post" action="registres_remises1.php">
...
<? 	if (isset($_REQUEST["action_l"])){
	$date=$_GET['date1'];$banque="";$date2=$_GET['date2'];
	$date_d1=$_GET['date1'];$date_d2=$_GET['date2'];
	$de1=$_GET['date1'];$de2=$_GET['date2'];
	}

	if(isset($_REQUEST["action"]))
	{ $date=dateFrToUs($_POST['date1']);$banque="";$date2=dateFrToUs($_POST['date2']);
	...
	$sql  = "SELECT * ";
	$sql .= "FROM registre_remises where (date between '$date' and '$date2') ORDER BY id;";
...
while($users_1 = fetch_array($users11)) { $id_r=$users_1["id"];$date_enc=$users_1["date"];$banque=$users_1["banque"];
			...
			while($users1_ = fetch_array($users1)) { 
				...
				$total_e=$total_e+$m_effet;
					...
					$sql .= "numero_remise_e = '" . $numero_remise_e . "', ";
					$sql .= "date_remise_e = '" . $date_remise_e . "', ";
					$sql .= "remise_e = '" . $remise_e . "' ";
					$sql .= "WHERE id = " . $id . ";";
					db_query($database_name, $sql);*/
