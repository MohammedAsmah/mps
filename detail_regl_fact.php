<?php
	...
	$action="recherche";$numero=$_GET["numero"];
...
<? 	
	...
	$sql  = "SELECT id,date_enc,date_cheque,r_impaye,date_impaye,client,client_tire,v_banque,numero_cheque,sum(m_cheque) As total_cheque ,
	sum(m_espece) As total_espece,numero_effet,sum(m_effet) As total_effet,sum(m_virement) As total_virement ";
	$sql .= "FROM porte_feuilles_factures where facture_n=$numero Group BY id;";
	$users11 = db_query($database_name, $sql);
...
<span style="font-size:24px"><?php echo "Detail reglements Facture : ".$numero."     Montant : ".$mm; ?></span>
...
while($users_1 = fetch_array($users11)) { 
			...
			$numero_effet=$users_1["numero_effet"];
			...
			<td><?php $dch= dateUsToFr($date_enc);print("<font size=\"1\" face=\"Comic sans MS\" color=\"000033\">$dch </font>");  ?></td>
...
<? $sql  = "SELECT * ";
	$sql .= "FROM porte_feuilles_factures where facture_n=$numero and r_impaye=1 and m_cheque<>0 ORDER BY date_impaye;";
...
<?php 
	...
	$sql  = "SELECT * ";
	$sql .= "FROM porte_feuilles_factures where facture_n=$numero and r_impaye_e=1 and m_effet<>0 ORDER BY id;";
