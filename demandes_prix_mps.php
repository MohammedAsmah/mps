<?php
	...
	if(isset($_REQUEST["action_"]) && $profile_id == 1) { 
		...
		switch($_REQUEST["action_"]) {
			...
			case "insert_new_user":
				...
				$result = mysql_query("SELECT commande FROM demandes_frs ORDER BY commande DESC LIMIT 0,1"); 
				...
				$result = mysql_query("SELECT eval FROM demandes_frs where (mois='$mois1' and annee='$annee') ORDER BY eval DESC LIMIT 0,1"); 
				$row = mysql_fetch_array($result); 
				$dir_eval = $row["eval"]+1;
				...
				$sql  = "INSERT INTO demandes_frs ( commande,date_e,client,ville,fax, vendeur, remise_10,remise_2,remise_3,evaluation,bb,destination,sans_remise ) VALUES ( ";
				$sql .= "'" . $cde . "', ";
				$sql .= "'" . $date . "', ";
				$sql .= "'" . $client . "', ";
				$sql .= "'" . $ville . "', ";
		...
		$date=$_POST['date'];$vendeur=$_POST['vendeur'];$action="recherche";$destination=$_POST['destination'];
			...
			$net = $_REQUEST["net"];if(isset($_REQUEST["active"])) {$active = $_REQUEST["active"];}
				...
				if ($mois=="Mar"){$mois1="MARS";}
				if ($mois=="Apr"){$mois1="AVRIL";}
				$result = mysql_query("SELECT eval FROM demandes_frs where (mois='$mois1' and annee='$annee') and active=1 ORDER BY eval DESC LIMIT 0,1"); 
				$row = mysql_fetch_array($result); 
				$dir_eval = $row["eval"]+1;
				if ($dir_eval<10){$dev="00".$dir_eval;}
				if ($dir_eval>9 and $dir_eval<100) {$dev="0".$dir_eval;}
			...
			case "delete_user":
			$sql  = "SELECT * ";
			$sql .= "FROM demandes_frs WHERE id = " . $_REQUEST["user_id"] . ";";
			$user = db_query($database_name, $sql); $users_ = fetch_array($user);
			$commande=$users_["commande"];$destination=$users_["destination"];
			$sql = "DELETE FROM demandes_frs WHERE id = " . $_REQUEST["user_id"] . ";";
			db_query($database_name, $sql);
			
			$sql = "DELETE FROM detail_demandes_frs WHERE commande = " . $commande . ";";
			db_query($database_name, $sql);
			$sql = "DELETE FROM detail_demandes_frs WHERE commande = " . $commande . ";";
			db_query($database_name, $sql);
	...
	?>
	<form id="form" name="form" method="post" action="bc_mps.php">
	...
	{$date=dateFrToUs($_POST['date']);$date_f=dateFrToUs($_POST['date']);$vendeur=$_POST['vendeur'];$destination=$_POST['destination'];
		...
		$sql  = "SELECT * ";
		$sql .= "FROM demandes_frs where date_e='$date' ORDER BY date_e;";
...
<script type="text/javascript"><!--
	function EditUser(user_id) { document.location = "demandes_prix_mps_details.php?user_id=" + user_id; }
--></script>
...
<span style="font-size:24px"><?php echo "Demandes"; ?></span>
<tr>
<td><?php echo dateUsToFr($date_f); ;?></td>
</tr><tr><? echo "<td><a href=\"demandes_prix_mps_details.php?vendeur=$vendeur&destination=$destination&date=$date_f&user_id=0\">Ajout Commande</a></td>";?>
</tr>
...
echo "<td><a href=\"demandes_prix_mps_details.php?date=$date_e&user_id=$id\">$client</a></td>";$ref=$users_["vendeur"];?>
<? echo "<td><a href=\"enregistrer_panier_demandes_mps.php?vendeur=$vendeur&client=$client&date=$date_e&numero=$commande\">Detail commande </a>";?>
