...
<? 	
	...
	$sql .= "FROM porte_feuilles where m_cheque<>0 and remise=0 and chq_f=1 GROUP BY id;";
...
while($users_1 = fetch_array($users11)) { 
			...
			$remise=$users_111["remise"];$numero_remise=$users_111["numero_remise"];$date_remise=$users_111["date_remise"];
			
			
			if ($remise==1){
			
			$sql = "UPDATE porte_feuilles SET ";
			$sql .= "remise = '" . $remise . "', ";$sql .= "numero_remise = '" . $numero_remise . "', ";$sql .= "date_remise = '" . $date_remise . "' ";
			$sql .= "WHERE id = " . $id . ";";
			...
			<? echo "<tr><td><a href=\"porte_feuilles_details1.php?date_cheque=$date_cheque1\">$date_cheque</a></td>";
				echo "<td><a href=\"porte_feuilles_details1.php?date_cheque=$date_cheque1\">$numero_cheque</a></td>";
			echo "<td><a href=\"porte_feuilles_details1.php?date_cheque=$date_cheque1\">$numero_remise</a></td>";
			echo "<td><a href=\"porte_feuilles_details1.php?date_cheque=$date_cheque1\">$date_remise</a></td>";
			?>
