<?php
	...
	<form id="form" name="form" method="post" action="etat_effets_globale_maj.php">
...
<? 	 $date=date("Y-m-d");$total=0;
	...
	$sql  = "SELECT * ";
	$sql .= "FROM porte_feuilles_factures where m_effet<>0 and (date_echeance between '$date' and '$date2') ORDER BY date_echeance;";
	$users11 = db_query($database_name, $sql);
...
while($users_1 = fetch_array($users11)) { $date_enc=$users_1["date_enc"];$vendeur=$users_1["vendeur"];$impaye=$users_1["impaye"];
			...
			$total_cheque=$users_1["total_cheque"];$total_espece=$users_1["total_espece"];$total_effet=$users_1["m_effet"];
